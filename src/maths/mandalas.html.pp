...
<div>
<p>
&emsp;The Tonči Juričev-Grgin mandala generation algorithm can be used to, <em>drumroll please</em>, generate mandalas like this one:
</p>
...
<p>
&emsp;It uses a very simple process (see <a href="#show-scans">below</a>), which can be used in any
	&lt;AsciiMath&gt;beta in NN ∖ {0}&lt;/AsciiMath&gt;-dimensional space, so, as an example, the same-sized mandala,
...
<strong>"How long does rendering a thing such as this take?"</strong>, one might ask.
  ...
  one need run the algorithm body many times, an acceptable amount I found was:<br />
	&lt;AsciiMath&gt;log(n_max) = |~log(prod_i size_i)~| + 1&lt;/AsciiMath&gt;,<br />
	(which still doesn't perfectly fill more complex corner setups), so for<!--'-->
	&lt;AsciiMath&gt;size = [900, 900, 900] => n_max = 10^10&lt;/AsciiMath&gt;,
	which takes about &lt;AsciiMath&gt;7 1/2 h&lt;/AsciiMath&gt; on my PC if I try to not cook it (and I generally do).
</p>
...
A different question to be asked is
	...
	Well, thanks to the wonders of the &lt;AsciiMath&gt;fr "Internet"&lt;/AsciiMath&gt;, you can! In most-to-least bad order:<br />
...
&emsp;I'm out of things to put here, because I'm not a cool person <em>en general</em>. <small>(What else did you expect, I do maths for fun)</small><br />
</p>
...
<div>
<p>
<span id="show-scans">Show algorithm scans (warning: bloody humongous).</span>
</p>
