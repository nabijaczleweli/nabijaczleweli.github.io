...
#include "blogn_t.h"
   ...
   <!-- RSS_PUB_DATE: "TODO" -->
#define POST_DATE      Tue, 29 Apr 2025 16:32:56 +0200
#define POST_POST_DATE


#define NUM(...)          <span class="roboto-unscale math-font-normal">__VA_ARGS__</span>
#define ONEOVER(...)      <math class="roboto-unscale math-font-normal"><mrow><mfrac><mi>1</mi><mi>__VA_ARGS__</mi></mfrac></mrow></math>
...
#define SCRIPTS     WORD_COUNT_SCRIPT FORCED_NEWLINE \
                    ...
                    <style>mjx-c::before { display: inline-block !important; /* https://github.com/mathjax/MathJax/issues/3355#issuecomment-2787780830 */ }</style>
BOILERPLATE(018. Mean-shift clustering — blognꞌt, 018. Mean-shift clustering, en-GB, STYLESHEETS FORCED_NEWLINE SCRIPTS)
<style>
...
HASHoklab-container *:nth-child(2),
.mathnospace > * {
...
HASHvideos_side_by_each video {
	display: block;
}


HASHPvol tr > *:nth-child(1),
.Ppop    tr > *:nth-child(1) {
	font-family: "Droid Sans Mono", monospace;
}
HASHPvol tr > *:nth-child(2) {
	text-align: right;
}
HASHPvol tr > *:nth-child(3) {
	text-align: left;
	padding-left: 0.5em;
}
HASHPvol,
...
BLOGN_T_HEADING(018. Mean-shift clustering)
...
an AMD Radeon RX 6500 XT (RADV NAVI24)/5800X3D/DDR4-3600 (weighted average NUM(14%) faster)
and a laptop with an iGPU (Intel(R) Iris(R) Xe Graphics (ADL GT2)/i5-1235U/SDDDR4 3200 MHz (0.3 ns)) (average NUM(3×) slower):
</p>
<table class="perf">
	<tr><th>(colours)    </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th></tr>
	<tr><th>5700 XT      </th>       <td>NUM( 79.46ms)</td>         <td>NUM(  3.35s)</td>       <td>NUM(   36.33s)</td></tr>
	<tr><th>6500 XT      </th>       <td>NUM( 91.24ms)</td>         <td>NUM(  4.00s)</td>       <td>NUM(   31.47s)</td></tr>
	<tr><th>i5-1235U iGPU</th>       <td>NUM(365.38ms)</td>         <td>NUM( 17.85s)</td>       <td>NUM(  118.90s)</td></tr>
	<tr></tr>
	<tr><th>(colours)    </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th></tr>
	<tr><th>5700 XT      </th>        <td>NUM(  1.11s)</td>         <td>NUM( 43.16s)</td>          <td>NUM(24.48s)</td></tr>
	<tr><th>6500 XT      </th>        <td>NUM(  1.01s)</td>         <td>NUM( 37.24s)</td>          <td>NUM(19.59s)</td></tr>
	<tr><th>i5-1235U iGPU</th>        <td>NUM(  4.41s)</td>         <td>NUM(165.40s)</td>          <td>NUM(60.20s)</td></tr>
</table>
<p class="continuing">
...
(All time figures are mean of at least NUM(5) measurements, usually more.)
</p>
<p class="indented continuing">
A CPU implementation processes that NUM(<!--'-->16'755)-colour image in NUM(5.5s) (on the i7-2600 which has 8 CPUs).
The others are a "don't even dream about it" scale.
</p>
<p class="indented continuation">
A naive GPU implementation
processes it in NUM(160ms),
times out after NUM(8s) after processing NUM(~50/242'931<!--'-->) colours,
and times out after NUM(8s) after processing NUM(~150/1'103'347) colours
...
This can be analysed as a NUM(2×)/NUM(1080×)/NUM(2450×) improvement and that is basically real.
</p>
<p class="indented">
...
HEADING(2, problem-statement, Problem statement)

<ol style="padding-left: 1em;" start="-3">
#define rC \color{var(--r)}r\color{initial}
#define gC \color{var(--g)}g\color{initial}
#define bC \color{var(--b)}b\color{initial}
#define RPAREN )
	<li>With operator MATHJAX(\circ)    as an element-wise application:   MATHJAX(\forall{i}   : (f \circ A)_{i}   = f(A_{i})),</li>
	<li>with operator MATHJAX(\odot)    as the Hadamard product:          MATHJAX(\forall{i}   : (A \odot B)_{i}   = A_{i}B_{i}),</li>
	<li>with          MATHJAX([])       as the Iverson bracket:           MATHJAX([x] = (1\text{ if }x\text{ otherwise }0)),</li>
	<li>with          MATHJAX(\bar{\ }) as the set of preceding naturals: MATHJAX(\bar{n} = [0, n RPAREN \cap \mathbb{Z}),</li>

	<li>let colour MATHJAX(C = ([0, 255] \cap \mathbb{Z})^3),</li>
	<li>given input MATHJAX(n)-dimensional image MATHJAX(I \in (C_i)_{i \in \mathbb{N}_+^n}),</li>
	<li>construct a map MATHJAX(f: C \to C) such that</li>
	<li>the output image MATHJAX(O) is given by MATHJAX(O = f \circ I).</li>
</ol>

<p class="continued">
Map MATHJAX(f) depends on deciding on a convex coordinate space for processing
MATHJAX(P \subset \mathbb{R}^3)
with mapping MATHJAX(p : C \to P \text{ & } p^{-1} : P \to C) such that MATHJAX(p^{-1}(p(x)) = x);
...
This gives MATHJAX(P = [0, 1] \times [\frac{-1}{2}, \frac{1}{2}] \times [\frac{-1}{2}, \frac{1}{2}]) and
</p>
<details id="oklab-container">
	<summary>Oklab MATHJAX(p\text{ & }p^{-1})</summary>
...
<!--"-->	(l, m, s) &= (l'^3, m'^3, s'^3)\\<!--"-->
        	rC' &= (l, m, s) \cdot (+4.0767416621, -3.3077115913, +0.2309699292)\\<!--'-->
        	gC' &= (l, m, s) \cdot (-1.2684380046, +2.6097574011, -0.3413193965)\\<!--'-->
        	bC' &= (l, m, s) \cdot (-0.0041960863, -0.7034186147, +1.7076147010)\\<!--'-->
\text{then}\\<!---->
p^{-1}(p) &= (\lfloor 255 rC'<!--'--> \rceil, \lfloor 255 gC'<!--'--> \rceil, \lfloor 255 bC' \rceil)\\<!--'-->
\end{align}
BIG_MATHJAX_END
</details>

<p class="continued">
#define PALETTE MATHJAX(palette)
#define MULFREQ MATHJAX(*freq)
Then MATHJAX(f) depends on radius MATHJAX(r) (with a functional range of MATHJAX([0, \sqrt{3}]) in Oklab),
and deciding on variation PALETTE or MULFREQ:
MULFREQ ends up on the center of mass by weighting by how many times it occurs in MATHJAX(I),
...
#define OVERBAR(...) {\mkern 1.5mu\overline{\mkern-1.5mu{__VA_ARGS__}\mkern-1.5mu}\mkern 1.5mu}
<p class="indented">
<!-- This can be generalised to transparent images (MATHJAX(C \times ([0, 255] \cap \mathbb{Z}))) by replicating the alpha channel directly: -->
This can be generalised to transparent images (MATHJAX(C \times OVERBAR(256))) by replicating the alpha channel directly:
MATHJAX(I_i = (rC, gC, bC) \to O_i = (rC'<!--'-->, gC'<!--'-->, bC'<!--'-->, a))
where
MATHJAX((rC'<!--'-->, gC'<!--'-->, bC'<!--'-->) = f((rC, gC, bC))).
Fully-transparent pixels should be discarded altogether.
</p>

<p class="indented continued">
Seeing as MATHJAX(D) is constant for either variation,
it follows trivially that MATHJAX(q_t = q_{t-1} \Leftrightarrow M_t = M_{t-1})
("the point didn't move between iterations" is equivalent to "the same points are within <!--"-->MATHJAX(r)<!--"--> as last iteration").
</p>
<p class="indented continuing">
It may be more convenient computationally to consider MATHJAX(D = p \circ Multiset(I)) with PALETTE reducing the multiplicity of each element to NUM(1).
...
#define FIG_URL(url, al, ...)                   \
	<figure>                                      \
		<a href=STR(url)><img alt=al src=STR(url) /></a> \
		<figcaption>__VA_ARGS__</figcaption>        \
	</figure>
#define FIG(f, al, ...) FIG_URL(/content/assets/blogn_t/f, al, __VA_ARGS__)

<div class="three-by-each sharp continuation" id="amogus">
	   FIG(018.01-2x2-100x.png,                    "a 2x2 image, the top row has two distinct purples, the bottom row is a third colour",
	       MATHJAX(I))<!--
	-->FIG(018.02-2x2.png.r=0.1-100x.png,          "a 2x2 image, all the same colour, a different purple",
	       MATHJAX(<!--"-->O(<!--"-->)MATHJAX(r=0.1,) MATHJAX(<!--"-->palette)<!--"-->))<!--
	-->FIG(018.03-2x2.png.r=0.1-mul-freq-100x.png, "the same, but another different shade",
	       MATHJAX(<!--"-->O(<!--"-->)MATHJAX(r=0.1,) MATHJAX(<!--"-->*freq)<!--"-->))
</div>

<p class="continuing">
For demonstrative simplicity, the source colours are all sampled from
<a href="/content/assets/blogn_t/018.04-l=0.797447133.png">the Oklab plane at NUM(l=0.797447133)</a>.
Due to <a href="//101010.pl/@nabijaczleweli/114197241319014485">quantisation unavoidable since MATHJAX(|P| \gg |C|)</a>
and them not <em>naturally</em> falling on that plane out of MATHJAX(p),
the intermediate MATHJAX(q_t)s aren't<!--'--> on that plane (but remain relatively close),
and those have been projected onto it (but this is why the points don't<!--'--> seem to follow a perfect geometric mean in 2D).
</p>

HEADING_S(4, demo-palette, class="continued", MATHJAX(palette, r=0.1: D=p \circ ((MH(CC), MH(4E), MH(FF)), (MH(FF), MH(2D), MH(FF)), (MH(FF), MH(2C), MH(A6)))))
<div class="three-by-each continuation">
	   FIG(018.05-l=0.797447133-start.png,  "three points highlighted at #CC4EFF (top left), #FF2DFF (top right), #FF2CA6 (bottom right), and an equal radius highlighted around each of them; the spacing is such that the radius of the top-right point includes all three points, but the others only include themselves and the top-right point ",
	       MATHJAX(q_0))<!--
	-->FIG(018.06-l=0.797447133-%27.png,    "the same, but new temporary points have migrated to #E63DFF, #EE38DF, #FF2DD0 (same order), and they're now meaningfully closer to each other. radiuses around them include all points",
	       MATHJAX(q_1 \leftarrow q_0))<!--
	-->FIG(018.07-l=0.797447133-%27%27.png, "the same, but they've all migrated to #EE38DF",
	       MATHJAX(q_2 \leftarrow q_1 \leftarrow q_0): top-right is done here since it saw the same points as last iteration)
</div>

HEADING_S(4, demo-mulfreq, class="continued", MATHJAX(*freq, r=0.1: D=p \circ \begin{pmatrix}(MH(CC), MH(4E), MH(FF)) & (MH(FF), MH(2D), MH(FF))\\(MH(FF), MH(2C), MH(A6)) & (MH(FF), MH(2C), MH(A6))\end{pmatrix}))
<div class="three-by-each">
	   FIG(018.08-l=0.797447133-mul-freq-start.png,  "the same three original points as before, but bottom-right has is now annotated 2x",
	       MATHJAX(q_0): this is similar to the PALETTE case, but MATHJAX(p((MH(FF), MH(2D), MH(FF)))) is represented twice)<!--
	-->FIG(018.09-l=0.797447133-mul-freq-%27.png,    "temporary points have migrated to #E63DFF, #335D0, #FF2DC1: this is similar but the latter two are meaningfully closer to the bottom-right point",
	       MATHJAX(q_1 \leftarrow q_0): top-left is the same, others are significantly closer to the doubled bottom-right)<!--
	-->FIG(018.09-l=0.797447133-mul-freq-%27%27.png, "they've all migrated to #F335D0",
	       MATHJAX(q_2 \leftarrow q_1 \leftarrow q_0))
</div>

<p class="indented">
In both cases MATHJAX(t) ends up being NUM(1) for MATHJAX(f((MH(FF), MH(2D), MH(FF)))) (top-right) and NUM(2) for the others.
A constant MATHJAX(t), smaller than would converge, <a href="//101010.pl/@nabijaczleweli/114048671396518223">may be used</a> to achieve more-visually-pleasing output,
but does not accomplish the functionality described by the original <del>paper</del> video.
</p>


HEADING(2, base-case, Base case)
<p class="indented continued">
...
<samp>RGB24</samp> MATHJAX([rC, gC, bC]) but
<samp>RGBA32</samp> either MATHJAX([a, bC, gC, rC]) or MATHJAX([rC, gC, bC, a]) depending on host endianness,
...
<samp>ABGR32</samp> on little-endian to get MATHJAX([rC, gC, bC, a]) and vice versa.
That's<!--'--> enough on SDL2.
</p>

<p class="indented continued">
The core insights informing the implementation is that MATHJAX(f) does not depend on the pixel position — just its colour —
and that MATHJAX(D) is constant for a given MATHJAX(I).
This means that computing MATHJAX(O) can take place in three distinct steps:
analysis by computing MATHJAX(D), pre-computing MATHJAX(f) for each distinct element in MATHJAX(D), mapping the pixels directly into MATHJAX(O).
The analysis step can happen just once per MATHJAX(I), regardless of variant and MATHJAX(r) selection.
</p>
<p class="indented continuing">
#define RRGGBB16 {rC}{rC}{gC}{gC}{bC}{bC}_{16}
Since MATHJAX(|C| = 2^{24}) (or, equivalently, MATHJAX(C \equiv RRGGBB16)),
and computer memory is many orders of magnitude larger than NUM(16Mi · any-reasonable-primitive-size),
a map from any colour can easily take the form of a look-up table of size 2<sup>24</sup>.
For example, the multiplicity map of MATHJAX(D) (<samp>frequencies</samp>) and the final map MATHJAX(f) (<samp>outputs</samp>)
are of types
<code>[u32; 0xFFFFFF + 1]</code>
and
<code>[[u8; 3]; 0xFFFFFF + 1]</code>
totalling a mere NUM(112MiB).
Extending this methodology to higher colour bit depths may require slower-but-more-memory-conscious collection types
<!-- (10-bit colour (MATHJAX(C = ([0, 1023] \cap \mathbb{Z})^3)) would grow them to to NUM(7GiB)). -->
(10-bit colour (MATHJAX(C = OVERBAR(1024)^3)) would grow them to to NUM(7GiB)).
A 32-bit integer for the multiplicity of each colour means that an entire NUM(4096<sup>2</sup>) image would need to be the same colour to overflow,
...
(with MATHJAX(p) as <samp>linear_srgb_to_oklab</samp>)
...
#include "blogn_t/018.01-analysis.rs.html"
</div>
<p class="continuation">
yields in <samp>known_rgbs</samp> a sequence of work items MATHJAX((RRGGBB16, p(I_{rC,gC,bC}), \text{multiplicity of}\ rC,gC,bC)).
</p>
...
<div class="codecontinued">
#include "blogn_t/018.02-pre-computation.rs.html"
</div>
<p class="continuing">
is a very direct implementation of the MATHJAX(q_t) search, with MATHJAX(t) being the iteration count of the <samp>loop</samp>
(the only and obvious optimisation is not computing the square root).
All elements of MATHJAX(D) are evaluated every time.
...
-->
<table class="perf">
	<tr><th>(colours)       </th>    <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>not     parallel</th>    <td>NUM(19.717s)</td>                 <td>NUM(11.533s)</td>                 <td>NUM(58.49%)</td></tr>
	<tr><th>NUM(8×) parallel</th>    <td>NUM( 6.098s)</td>                 <td>NUM( 3.974s)</td>                 <td>NUM(65.16%)</td></tr>
	<tr><th>(parallel speed-up)</th> <td>NUM( 3.233×)</td>                 <td>NUM( 2.903×)</td>                 <td>NUM(-6.66 p.p.)</td></tr>
</table>
<p class="continuing">
#define DUM(__VA_ARGS__) NUM(__VA_ARGS__<sub>d</sub>)
...
I'd<!--'--> thought that MULFREQ converged NUM(~40%) faster due to the specific data contained in the test image, which makes direct comparison of the two difficult;
henceforth, images annotated DUM() are the
...
making MULFREQ equivalent to PALETTE:
</p>
...
-->
<table class="perf">
	<tr><th>(colours)       </th>    <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>not     parallel</th>    <td>NUM( 9.812s)</td>                 <td>NUM( 6.853s)</td>                 <td>NUM(69.84%)</td></tr>
	<tr><th>NUM(8×) parallel</th>    <td>NUM( 2.924s)</td>                 <td>NUM( 2.012s)</td>                 <td>NUM(68.79%)</td></tr>
	<tr><th>(parallel speed-up)</th> <td>NUM( 3.355×)</td>                 <td>NUM( 3.406×)</td>                 <td>NUM(-1.05 p.p.)</td></tr>
</table>
<p class="continuing">
One would expect (more floating-point operations executed, more data loaded) MULFREQ to be, at best, the same speed as PALETTE,
but the optimiser <a href="//github.com/rust-lang/rust/issues/138953"><em>actually</em> makes the PALETTE implementation NUM(30%) worse</a>.
Science cannot explain this.
</p>

#define DEL(...)
DEL(
<!-- Measurements from f39afc3 -->
<!--
	posterise_ncpus:
5.351357136s: MSC 16755 RGBs @ r=0.02
5.422878206s: MSC 16755 RGBs @ r=0.02
5.425513149s: MSC 16755 RGBs @ r=0.02
5.406402345s: MSC 16755 RGBs @ r=0.02
5.45930857s: MSC 16755 RGBs @ r=0.02
3.929426471s: MSC 16755 RGBs @ r=0.02 * freq
4.037671012s: MSC 16755 RGBs @ r=0.02 * freq
3.806861657s: MSC 16755 RGBs @ r=0.02 * freq
3.832018126s: MSC 16755 RGBs @ r=0.02 * freq
4.03428836s: MSC 16755 RGBs @ r=0.02 * freq
-->
<!--
	posterise_linear:
18.752012053s: MSC 16755 RGBs @ r=0.02
18.53678173s: MSC 16755 RGBs @ r=0.02
19.295162742s: MSC 16755 RGBs @ r=0.02
18.480305399s: MSC 16755 RGBs @ r=0.02
18.475141467s: MSC 16755 RGBs @ r=0.02
12.003899739s: MSC 16755 RGBs @ r=0.02 * freq
12.032045234s: MSC 16755 RGBs @ r=0.02 * freq
11.776490241s: MSC 16755 RGBs @ r=0.02 * freq
12.429187699s: MSC 16755 RGBs @ r=0.02 * freq
11.5939457s: MSC 16755 RGBs @ r=0.02 * freq
-->
<table class="perf">
	<tr><th>(colours)       </th>    <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>not     parallel</th>    <td>NUM(18.708s)</td>                 <td>NUM(11.967s)</td>                 <td>NUM(64%)</td></tr>
	<tr><th>NUM(8×) parallel</th>    <td>NUM( 5.413s)</td>                 <td>NUM( 3.928s)</td>                 <td>NUM(72%)</td></tr>
	<tr><th>(parallel speed-up)</th> <td>NUM( 3.456×)</td>                 <td>NUM( 3.047×)</td></tr>
</table>

<!--
	posterise_ncpus:
5.84092664s: MSC 16756 RGBs @ r=0.02
5.596834507s: MSC 16756 RGBs @ r=0.02
5.462309669s: MSC 16756 RGBs @ r=0.02
5.351804493s: MSC 16756 RGBs @ r=0.02
5.546939041s: MSC 16756 RGBs @ r=0.02
4.42084125s: MSC 16756 RGBs @ r=0.02 * freq
4.377727689s: MSC 16756 RGBs @ r=0.02 * freq
4.332460352s: MSC 16756 RGBs @ r=0.02 * freq
4.600961547s: MSC 16756 RGBs @ r=0.02 * freq
4.277199915s: MSC 16756 RGBs @ r=0.02 * freq
-->
<!--
	posterise_linear:
18.401484431s: MSC 16756 RGBs @ r=0.02
18.408581478s: MSC 16756 RGBs @ r=0.02
18.526020998s: MSC 16756 RGBs @ r=0.02
19.013256516s: MSC 16756 RGBs @ r=0.02
18.987489683s: MSC 16756 RGBs @ r=0.02
15.026657924s: MSC 16756 RGBs @ r=0.02 * freq
14.337646923s: MSC 16756 RGBs @ r=0.02 * freq
14.752311197s: MSC 16756 RGBs @ r=0.02 * freq
14.398518347s: MSC 16756 RGBs @ r=0.02 * freq
14.388174485s: MSC 16756 RGBs @ r=0.02 * freq
-->
<table class="perf">
	<tr><th>(colours)       </th>    <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>not     parallel</th>    <td>NUM(18.667s)</td>                 <td>NUM(14.581s)</td>                 <td>NUM(78%)</td></tr>
	<tr><th>NUM(8×) parallel</th>    <td>NUM( 5.560s)</td>                 <td>NUM( 4.402s)</td>                 <td>NUM(79%)</td></tr>
	<tr><th>(parallel speed-up)</th> <td>NUM( 3.358×)</td>                 <td>NUM( 3.312×)</td></tr>
</table>
)
...
(with MATHJAX(p^{-1}) as <samp>oklab_to_linear_srgb</samp>):
</p>
<div class="codecontinued">
#include "blogn_t/018.03-output-mapping.rs.html"
</div>
...
<a href="//101010.pl/@nabijaczleweli/114230202476246959">practically every Minecraft texture has </a><a
	href="//paste.sr.ht/~nabijaczleweli/dba1e5492c6430f2a3f062819cbbda5eabe558af#Minecraft%201.20.4%20texture%20distinct%20colour%20counts">NUM(&lt;100)</a><a
...
But since each MATHJAX(f(c)) inspects each distinct element of MATHJAX(D) MATHJAX(t) times,
the total complexity is MATHJAX(\propto (\overline{Set(D)})^2 t_{avg})
which for more colourful images this simply won't<!--'--> do.
</p>
<div class="three-by-each sharp continuation">
	   FIG_URL(/assets/nabijaczleweli.png,                "my avatar, full-colour",
	       MATHJAX(I))<!--
	-->FIG(018.10-nabijaczleweli.png.r=0.02.png,          "my avatar, most of the detail has been replaced with flat colours",
	       MATHJAX(<!--"-->O(<!--"-->)MATHJAX(r=0.02,) MATHJAX(<!--"-->palette)<!--"-->)<br />
	       MATHJAX(\Sigma t = 440133))<!--
	-->FIG(018.11-nabijaczleweli.png.r=0.02-mul-freq.png, "my avatar, more detail has been preserved",
	       MATHJAX(<!--"-->O(<!--"-->)MATHJAX(r=0.02,) MATHJAX(<!--"-->*freq)<!--"-->)<br />
	       MATHJAX(\Sigma t = 358543))
</div>
<p class="continuation">
To wit: the unified Minecraft Alpha v1.2.2 block texture atlas has NUM(400) distinct colours and converges
in MATHJAX(\Sigma t = 423378)/MATHJAX(\Sigma t = 664403),
respectively with the same settings, in NUM(&lt;1ms).
</p>
...
Output mapping is a little sub-optimal for large images, since it re-computes MATHJAX(p'<!--'-->) for every pixel every time
(<a href="//nabijaczleweli.bandcamp.com/album/cinque">a large (NUM(2160<sup>2</sup>)) image</a> takes NUM(176ms) to map).
By <a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/4df2b1ce034c3afd5d080a2fc341aa5096724356/item/src/main.rs#L465">in situ applying MATHJAX(p'<!--'-->)</a>
to turn <samp>[Lab; 0xFFFFFF + 1]</samp> into <samp>[[u8; 3 + <var>padding</var>]; 0xFFFFFF + 1]</samp>:
</p>
<div class="codecontinued">
#include "blogn_t/018.04-output-mapping-parboiled.rs.html"
</div>
...
-->
<p class="continuation">
This drops to NUM(16ms), and correspondingly less on smaller images — immaterial on the scale of <em>seconds</em>.
</p>
...
FIG(018.12-graphml-cpu+interstitial.png, "", )
</div>
...
HEADING_S(2, vulkanisation, class="continuation", rubber to the road)
...
<ol>
	...
	<li>it appeared to competently implement both a direct Vulkan binding
	    and a NUM(10%) more structured and NUM(40%) less verbose "builder" scheme that supposedly avoided dangling references
...
<div class="codecontinued">
#include "blogn_t/018.05-build.rs.html"
</div>
<p class="continuing">
...
Since <samp>known_rgbs</samp> could have anywhere MATHJAX(\in [1, 2^{24}]) elements (but is very unlikely to have actually that many),
...
<div class="codecontinued">
#include "blogn_t/018.06-posterise.hlsl.html"
</div>
<p class="continuing">
...
#define SB <abbr title="StructuredBuffer">SB</abbr>
<div class="codecontinued">
#include "blogn_t/018.07-driver.rs.html"
</div>
...
<p class="indented continuing">
It uses MATHJAX(\text{numthreads} = (1, 1, 1), \text{dispatch}(k, 1, 1)), which is fine for its application.
On expert advice for "why is this so damn slow still",
I'd<!--'--> naively upgraded to MATHJAX(\text{numthreads} = (64, 1, 1), \text{dispatch}(k, 1, 1))
(where MATHJAX(k ={\ })<samp>known_rgbs.len()</samp>),
which exposes a glaring bug: the "threads" dispatched in each dimension are
MATHJAX(\prod_{x \in \text{numthreads} \odot \text{dispatch}} [0, x RPAREN\ (\text{all in}\ \mathbb{Z})),
so, in this case,
MATHJAX([0, 64n RPAREN \times [0, 1 RPAREN \times [0, 1 RPAREN)
instead of the desired
MATHJAX([0, \mathbin{\approx}k RPAREN \times [0, 1 RPAREN^2)
...
the description is incredibly wordy and disconnected from where the other "<!--"-->NUM(256)<!--"-->" is,
defines only the edge-case when MATHJAX(k \bmod 64 = 0),
...
MATHJAX(\text{numthreads} = (64, 1, 1), \text{dispatch}(\lceil\frac{k}{64}\rceil, 1, 1))
which yields
MATHJAX([1, k+\varepsilon RPAREN \times [0, 1 RPAREN^2)
where
MATHJAX(\varepsilon \in [0, 64 RPAREN).
...
<div class="codecontinued">
#include "blogn_t/018.08-posterise.id.patch.hlsl.html"
</div>
<p class="continuing">
...
<p class="indented continuation">
Why NUM(64) specifically? I measured and everything else is worse. Something to do with compute unit sizes or whatever? Unclear.
</p>

<p class="indented continued">
That bit about the <cite>Example</cite> having one fault? Not true actually. It doesn't<!--'--> mention fence resetting.
If you're like me, god forbid, and don't intently read through NUM(60%) of the <cite>Vulkan Tutorial</cite>
...
<p class="indented continued">
One may note the mismatch of <samp>outputs</samp> being <samp>SB&lt;float3></samp> but then also <samp>[[f32; 4]; …]</samp>,
as well as <samp>params</samp> also being in a <samp>SB</samp> even though there's<!--'--> only one of them, ever.
This is due to a confluence of two HLSL oddities:
</p>
<ul>
	<li>arrays have either a NUM(4)-byte or a NUM(16)-byte alignment</li>
	<li>globals not part of a SB are collected and encapsulated into a single global you can't<!--'--> control</li>
</ul>
<p class="continuing">
thus, <samp>SB&lt;float3></samp> is actually <samp>SB&lt;(float3, <var>4 unused bytes</var>)</samp>,
which also means that <samp>known_rgbs_bundle</samp> is packed optimally.
Arrays whose elements are NUM(1)/NUM(2) scalars (NUM(4)/NUM(8) bytes) are NUM(4)-byte-aligned.
Larger arrays are NUM(16)-byte-aligned.
...
-->
<table class="perf">
	<tr><th>(colours)            </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>±σ</th></tr>
	<tr><th>5700 XT              </th> <td>NUM(132.734ms)</td>               <td>NUM(464µs)</td></tr>
	<tr><th>(speed-up vs NUM(8×))</th> <td>NUM(40.782×)</td>                 <td></td></tr>
</table>
...
<code>known_rgbs_bundle</code> is only as big as <code>known_rgbs</code> (distinct colours in MATHJAX(I)),
...
but my Win32 amdgpu-equivalent driver kills the shader and returns <code>TIMEOUT</code> if it takes longer than NUM(~8s)
...
<p class="indented continued">
By observing that <code>length(v)</code> MATHJAX(= \sqrt{v_x^2 + v_y^2 + v_z^2} = \sqrt{v_xv_x + v_yv_y + v_zv_z} = \sqrt{v \cdot v} = ) <samp>sqrt(dot(v, v))</samp>
...
<div class="codecontinued">
#include "blogn_t/018.09-posterise.obvious.patch.hlsl.html"
</div>
<!--
...
<table class="perf left">
	<tr><th>(colours)             </th> <th>NUM(<!--'-->16'755), PALETTE</th></tr>
	<tr><th>5700 XT               </th> <td>NUM(128.871ms)</td></tr>
	<tr><th>(speed-up vs previous)</th> <td>NUM(2.91%)</td></tr>
</table>
...
HEADING_S(2, baseline, class="continuing", base-line)
<p class="continuation">
port is implementing MULFREQ.
...
<div class="codecontinued">
#include "blogn_t/018.10-baseline-posterise_kernel.hlsl.inc.hlsl.html"
</div>
...
<div class="codecontinued">
#include "blogn_t/018.11-baseline-posterise.hlsl.html"
</div>
...
<div class="codecontinued">
#include "blogn_t/018.12-baseline-posterise_mul.hlsl.html"
</div>

<p class="continued">These expand to</p>
<div class="codecontinued">
#include "blogn_t/018.13-baseline-posterise-inlined.hlsl.html"
</div>
<p class="continuing">and</p>
<div class="codecontinued">
#include "blogn_t/018.14-baseline-posterise_mul-inlined.hlsl.html"
</div>
...
Nominally, Vulkan only has <a href="//docs.vulkan.org/spec/latest/chapters/limits.html#:~:text=4%20(Vulkan%20Core)">NUM(4)</a> bindings in baseline,
...
but the value is marginal since all GPUs mentioned heretofore support NUM(32).
</p>
...
<div class="codecontinued">
#include "blogn_t/018.15-baseline-driver.rs.html"
</div>
...
<table class="perf">
	<tr><th>(colours)             </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT               </th> <td>NUM(128.854ms)</td>               <td>NUM(118.864ms)</td>               <td>NUM(92.25%)</td></tr>
	<tr><th>(speed-up vs previous)</th> <td>NUM(&lt;0.015%)</td></tr>
	<tr><th>(speed-up vs NUM(8×)) </th> <td>NUM(42.010×)</td>                 <td>NUM(33.047×)</td>                 <td>NUM(-27.09 p.p.)</td></tr>
	<tr></tr>
	<tr><th>(colours)             </th> <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT               </th> <td>NUM(130.972ms)</td>               <td>NUM(151.862ms)</td>               <td>NUM(115.95%)</td></tr>
	<tr><th>(speed-up vs NUM(8×)) </th> <td>NUM(42.450×)</td>                 <td>NUM(28.986×)</td>                 <td>NUM(-47.16 p.p.)</td></tr>
</table>

<p class="continuation">
MULFREQ being NUM(16%) slower than PALETTE finally matches the expectation based on complexity;
...
-->
#define TIMEOUTS(howmany, correct) [time-out; NUM(howmany)/NUM(correct)]
#define TIMEOUT(howmany, correct)  TIMEOUTS(~howmany, correct)
#define TIMEOUT_PERF_TFOOT(colspan_val) \
	<tfoot class="label"><tr><td colspan=colspan_val>\
		Computation of cells labelled <q>time-out</q> was terminated by the driver after like NUM(8-11s); \
		progress corresponds to <em>how many distinct output colours</em> <span style="white-space: nowrap;">(MATHJAX(\overline{Set(O)}))</span> were produced \
		vs. a correct implementation; \
		this is a very vague proxy measured for a very vague amount of time. \
	</td></tr></tfoot>
<table class="perf left">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>       <td>NUM(129.33ms)</td>          <td>NUM(6.606s)</td>    <td>TIMEOUT(106, 155)</td> <td>NUM(1.262ms)/NUM(245.740ms)</td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM(121.11ms)</td>          <td>NUM(4.357s)</td>    <td>TIMEOUT(145, 152)</td> <td>NUM(3.066ms)/NUM(439.297ms)</td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>         <td>NUM(1.456s)</td>     <td>TIMEOUTS(0, 220)</td>  <td>TIMEOUT( 225, 6505)</td> <td>NUM(54.549ms)</td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM(373.43ms)</td>     <td>TIMEOUT(79, 132)</td>  <td>TIMEOUT(1627, 5075)</td> <td>NUM(42.771ms)</td></tr>
	TIMEOUT_PERF_TFOOT(4)
</table>

<div class="singles">
FIG(018.13-graphml-baseline.png, "", )
</div>
...
HEADING_S(2, memory-types, class="continuing", memory types)
<p class="continuation">
...
Thus, "a <!--"-->MATHJAX(Q)<!--"--> memory" is commonly taken to mean "the first memory type that satisfies predicate <!--"-->MATHJAX(Q)<!--"-->",
...
<table class="memtype">
	<caption>AMD Radeon RX 5700 XT THICC II [<a href="//en.wikipedia.org/wiki/Radeon_RX_5000_series#Desktop">NUM(8GiB)</a> on device, NUM(16GiB) on host]</caption>
	<tr><td>NUM(7920MiB)</td>	<td><!--"-->DEVICE_LOCAL<!--"--></td></tr>
	<tr><td>NUM(7909MiB)</td>	<td><!--"-->HOST_VISIBLE | HOST_COHERENT<!--"--></td></tr>
	<tr><td>NUM( 256MiB)</td>	<td><!--"-->DEVICE_LOCAL | HOST_VISIBLE  | HOST_COHERENT<!--"--></td></tr>
	<tr><td>NUM(7909MiB)</td>	<td><!--"-->HOST_VISIBLE | HOST_COHERENT | HOST_CACHED<!--"--></td></tr>
	<tr><td colspan=2>(these repeat with <samp>| DEVICE_COHERENT_AMD | DEVICE_UNCACHED_AMD</samp> set)</td></tr>
</table>

<table class="memtype">
	<caption>AMD Radeon RX 6500 XT (RADV NAVI24) [<a href="//en.wikipedia.org/wiki/Radeon_RX_6000_series#Desktop">NUM(4GiB)</a> on device, NUM(64GiB) on host]</caption>
	<tr><td>NUM( 4080MiB)</td> <td><!--"-->DEVICE_LOCAL<!--"--></td></tr>
	<tr><td>NUM( 4080MiB)</td> <td><!--"-->DEVICE_LOCAL<!--"--></td></tr>
	<tr><td>NUM(32110MiB)</td> <td><!--"-->HOST_VISIBLE | HOST_COHERENT<!--"--></td></tr>
	<tr><td>NUM( 4080MiB)</td> <td><!--"-->DEVICE_LOCAL | HOST_VISIBLE  | HOST_COHERENT<!--"--></td></tr>
	<tr><td>NUM( 4080MiB)</td> <td><!--"-->DEVICE_LOCAL | HOST_VISIBLE  | HOST_COHERENT<!--"--></td></tr>
	<tr><td>NUM(32110MiB)</td> <td><!--"-->HOST_VISIBLE | HOST_COHERENT | HOST_CACHED<!--"--></td></tr>
	<tr><td>NUM(32110MiB)</td> <td><!--"-->HOST_VISIBLE | HOST_COHERENT | HOST_CACHED<!--"--></td></tr>
	<tr><td colspan=2>(these repeat, after uniquification, with <samp>| DEVICE_COHERENT_AMD | DEVICE_UNCACHED_AMD</samp> set)</td></tr>
</table>

<table class="memtype">
	<caption>Intel(R) Iris(R) Xe Graphics (ADL GT2) [unified NUM(64GiB)]</caption>
	<tr><td>NUM(32007MiB)</td>	<td><!--"-->DEVICE_LOCAL<!--"--></td></tr>
	<tr><td>NUM(32007MiB)</td>	<td><!--"-->DEVICE_LOCAL | HOST_VISIBLE | HOST_COHERENT<!--"--></td></tr>
	<tr><td>NUM(32007MiB)</td>	<td><!--"-->DEVICE_LOCAL | HOST_VISIBLE | HOST_COHERENT | HOST_CACHED<!--"--></td></tr>
	<tr><td>NUM(32007MiB)</td>	<td><!--"-->DEVICE_LOCAL | PROTECTED<!--"--></td></tr>
...
It appears that GPUs can only see up to NUM(32GiB) of host memory (or, indeed, memory at all), regardless of how much is actually attached
...
<p class="indented continuing">
#define SIZEOF(...) \operatorname{sizeof}(\texttt{__VA_ARGS__})
...
(taking up MATHJAX(SIZEOF(params) + k (SIZEOF(}\text{(}\texttt{float3}\text{, }\texttt{uint}\text{)) + SIZEOF(uint))), so NUM(20B) per unique colour).
This is a precious resource, so <samp>outputs</samp> can't<!--'--> fit here
(MATHJAX(2^{24} SIZEOF(float4)) is the whole MATHJAX(256\text{MiB})!),
but from the perspective of the GPU, <samp>outputs</samp> is write-once write-only,
and the last iteration of MATHJAX(f), functionally, <em>uploads</em> the result,
...
<table class="perf">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(125.566ms)</td>               <td>NUM(116.503ms)</td>               <td>NUM( 92.78%)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(  3.288ms)</td>               <td>NUM(  2.361ms)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(    2.55%)</td>               <td>NUM(    1.99%)</td></tr>
	<tr></tr>
	<tr><th>(colours)              </th> <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(127.674ms)</td>               <td>NUM(148.930ms)</td>               <td>NUM(116.65%)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(  3.298ms)</td>               <td>NUM(  2.931ms)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(    2.52%)</td>               <td>NUM(    1.93%)</td></tr>
</table>


HEADING(2, branchless, who up branchin they execution)
...
so can
<a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/ad9926e0813ebbcc31b5c22f30e97d1b54ffedd1/item/src/posterise_mul.hlsl">MULFREQ's<!--'--></a>
...
<div class="codecontinued">
#include "blogn_t/018.16-branchless-posterise.hlsl.html"
</div>
<div class="codecontinued">
#include "blogn_t/018.17-branchless-posterise_mul.hlsl.html"
</div>
<div class="codecontinued">
#include "blogn_t/018.18-branchless-posterise_kernel.hlsl.html"
</div>
<p class="continuing">
which expand to, effectively,
</p>
<div class="codecontinued">
#include "blogn_t/018.19-branchless-posterise-inlined.hlsl.html"
</div>
<div class="codecontinued">
#include "blogn_t/018.20-branchless-posterise_mul-inlined.hlsl.html"
</div>
<p class="continuing">
yielding
</p>
...
<table class="perf">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(123.416ms)</td>               <td>NUM(113.429ms)</td>               <td>NUM( 91.91%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(    1.71%)</td>               <td>NUM(    2.64%)</td>               <td>NUM(  0.87 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(    4.22%)</td>               <td>NUM(    4.57%)</td>               <td>NUM(  0.34 p.p.)</td></tr>
	<tr></tr>
	<tr><th>(colours)              </th> <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(123.776ms)</td>               <td>NUM(147.078ms)</td>               <td>NUM(118.83%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(    3.05%)</td>               <td>NUM(    1.24%)</td>               <td>NUM( -2.18 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(    5.49%)</td>               <td>NUM(    3.15%)</td>               <td>NUM( -2.89 p.p.)</td></tr>
</table>

<p class="continued">
For reference, "worse" means, giving MULFREQ <samp>BRANCHLESS_ADD</samp> = <samp>1</samp> and <samp>FREQ</samp> = <samp>matched * freq</samp>:
</p>
<!-- matched * freq
[src\main.rs:163:27] possible.swap_remove(i as usize) = ".\\00.png"
...
-->
<table class="perf">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th>  <th style="padding-left: 1em;">DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(116.641ms)</td>               <td>NUM( 94.51%)</td>       <td                           >NUM(148.654ms)</td>               <td>NUM(101.07%)</td></tr>
	<tr><th>(speed-up vs not that) </th> <td>NUM(   -2.83%)</td>               <td>NUM(  2.60 p.p.)</td>   <td                           >NUM(   -1.07%)</td>               <td>NUM(-17.75 p.p.)</td></tr>
</table>


HEADING(2, grid, Hitting the griddy? In this economy?)
...
except that since the underlying set of MATHJAX(Multiset(D)) is constant,
the elements of MATHJAX(D) can be assigned to constant cells
(or, equivalently but more importantly, cells can be said to contain a constant subset of MATHJAX(D),
 where no two cells share any elements and the union of all cells is MATHJAX(D))
a priori,
but each iteration of MATHJAX(q_t) moves around, so the cell it's<!--'--> found in will differ.
</p>
<p class="indented continuing">
...
<div class="one-by-each continuation">
	FIG(018.14-unscaled+scaled-ortho-front.png, "Coloured point clouds labelled α, β, γ, δ, ε, in two rows: the bottom with a bounding box around the cloud which itself takes up like a twentieth of the visible volume, the top with the bounded bit blown up",
	    Bottom: MATHJAX(D), bounds of data out-lined; top: bounding box scaled to NUM(1<sup>3</sup>); front view)
	FIG(018.15-unscaled+scaled-ortho-top.png,   "The same, from a different angle",
	    Top view)
</div>
...
-->
<p class="indented continued">
As is hopefully obvious from the figures, the range of MATHJAX(P) actually used by any given input tends to be quite narrow,
so subdividing MATHJAX(P) directly will yield poor occupancy.
Indeed, for the datasets visualised above:
</p>
#define EXAMPLE_Α_STYLE style="color: #4b8494;"
#define EXAMPLE_Β_STYLE style="color: #94a088;"
#define EXAMPLE_Γ_STYLE style="color: #735d56;"
#define EXAMPLE_Δ_STYLE style="color: #15090d;" class="dark-invert"
#define EXAMPLE_Ε_STYLE style="color: #b57688;"
<table id="Pvol">
	<tr><th></th>                                              <th>volume</th>         <th colspan=5>bounding box</th></tr>
	<tr><td EXAMPLE_Α_STYLE>α</td> <td>NUM(≈0.041316)</td> <td>MATHJAX([0.13635494, 1.0]       )</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.13823886, 0.02675569])</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.16005541,  0.12989232])</td></tr>
	<tr><td EXAMPLE_Β_STYLE>β</td> <td>NUM(≈0.026181)</td> <td>MATHJAX([0.27619627, 0.9986911] )</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.15646803, 0.04568255])</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.037055194, 0.1422047] )</td></tr>
	<tr><td EXAMPLE_Γ_STYLE>γ</td> <td>NUM(≈0.034417)</td> <td>MATHJAX([0.0,        0.9665335] )</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.06452984, 0.12635213])</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.10584019,  0.08071107])</td></tr>
	<tr><td EXAMPLE_Δ_STYLE>δ</td> <td>NUM(≈0.107448)</td> <td>MATHJAX([0.0,        0.68679345])</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.18539383, 0.20771612])</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.2556958,   0.14228153])</td></tr>
	<tr><td EXAMPLE_Ε_STYLE>ε</td> <td>NUM(≈0.057179)</td> <td>MATHJAX([0.31232694, 1.0]       )</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.03692481, 0.27541596])</td> <td>MATHJAX(\times)</td> <td>MATHJAX([-0.09577876,  0.17043555])</td></tr>
</table>
<p class="continuation">
so, with Oklab MATHJAX(P)'s<!--'--> volume being NUM(1), the average bounds are just NUM(5.33%) of that!
</p>

<p class="indented continued">
For a cartesian grid of dimension MATHJAX(G \in \mathbb{N}_+) we can define a bijective map MATHJAX(g : P \to [0, G RPAREN^3) such that
</p>
...
<p class="continuation">
#define TRUNC \operatorname{trunc}
a bare definition of MATHJAX(g) (without MATHJAX(\min)) would also include MATHJAX({G}^3) in the codomain
for points on the far extrema of the bounding box;
we abuse the fact that we don't<!--'--> <em>actually</em> use MATHJAX(g^{-1}(g(p))) as data, just as optimisation metadata,
to make it much more useful (the error induced is imperceptible after quantisation later):
<!-- the grid cell each MATHJAX(p) falls in is given by MATHJAX(TRUNC(g(p)) \in ([0, G RPAREN \cap \mathbb{Z})^3). -->
the grid cell each MATHJAX(p) falls in is given by MATHJAX(TRUNC(g(p)) \in \bar{G}^3).
<!-- The points within each cell MATHJAX(h : ([0, G RPAREN \cap \mathbb{Z})^3 \to \mathcal{P}(D)) can then be obtained by computing -->
The points within each cell MATHJAX(h : \bar{G}^3 \to \mathcal{P}(D)) can then be obtained by computing
MATHJAX(h(x) = \{p | p \in D \wedge TRUNC(g(p)) = x\});
the desired properties
MATHJAX(h(x) \cap h(y) \ne \varnothing \Leftrightarrow x = y \text{ & }
        \bigcup (h \circ {TRUNC} \circ g \circ D) = D)
are preserved under these mappings.
</p>

<div class="three-by-each directly continuation">
	   FIG(018.18-grid-ortho-G=100-x.png, "Blown up point cloud α overlaid with a 100x100 grid",
	       MATHJAX(G = 100), orthographic projection along MATHJAX(x))<!--
	-->FIG(018.19-grid-ortho-G=100-y.png, "The same, alongside different axis",
	       MATHJAX(y))<!--
	-->FIG(018.20-grid-ortho-G=100-z.png, "Likewise",
	       MATHJAX(z))
</div>

<div class="singles continuation">
	FIG(018.21-grid-ortho-G=10-persp.png, "Point cloud α in 3D overlaid with a 10x10x10 grid",
	    MATHJAX(G = 10)
	    (it's<!--'--> <a href="//101010.pl/@nabijaczleweli/114263550582075495">practically impossible</a> to get a useful non-axial non-ortho render at MATHJAX(100)))
</div>


HEADING(3, grid-bbox, box bondage 🥺)

<p class="indented continued">
Computing MATHJAX((B_m, B_M)) is more-usefully analysed as a MATHJAX((\min, \max)) fold
over MATHJAX(D) starting at MATHJAX((+\infty^3, -\infty^3)).
To this end, MATHJAX(\min) and MATHJAX(\max) are available as atomics —
HLSL calls them
<a href="//learn.microsoft.com/en-us/windows/win32/direct3dhlsl/interlockedmin"><samp>InterlockedMin</samp></a>/\
<a href="//learn.microsoft.com/en-us/windows/win32/direct3dhlsl/interlockedmax">-<samp>Max</samp></a>
...
but only on integers, which coordinates in MATHJAX(D) aren't<!--'-->.
</p>
<p class="indented continuing">
But they are IEEE754 <samp>float</samp>s, which have a well-behaved structure when interpreted as-if they were integers.
Indeed, if MATHJAX(P) were in the form MATHJAX([0, x]^3), no further processing would be required,
...
But it isn't<!--'-->, so we desire a bijection MATHJAX(u : float \to uint \text{ or } int) monotonic in MATHJAX((-\infty, +\infty)),
...
(and <a href="//github.com/kripken/bullet/blob/7e5c986/src/BulletMultiThreaded/GpuSoftBodySolvers/DX11/btSoftBodySolver_DX11.cpp#L1178-L1179">MATHJAX(u^{-1})</a>),
for 32-bit <samp>float</samp>s and <samp>uint</samp>s
(notationally, this is where real math (Wikipedia lists operators in MATHJAX(\LaTeX)) ends and "computer science" (Wikipedia l. op. in <samp>Courier</samp>) begins):
</p>

<div class="mathnospace" style="text-align: center;">
#define UREV u<sup style="display: inline-block; width: 1.43em;">-1</sup>
<pre>\
u(f)<!--" -->   = asuint(f) ^ ((~(asuint(f) >> 31) + 1) | 0x8000`0000) <!--"-->
UREV(u)<!--"--> = asfloat(u ^ (( (       u  >> 31) - 1) | 0x8000`0000))<!--"-->
</pre>
...
<p class="indented continued">
Within the MATHJAX((-\infty, +\infty)) domain,
IEEE754 <samp>float</samp>s consist of MATHJAX((1\ \text{sign bit})(31\ \text{bits of distance from}\ 0\ \text{in natural }\texttt{uint}\text{ sort order})).
...
The MATHJAX(l) axis from example <samp EXAMPLE_Β_STYLE>β</samp> can thus be transcoded as
</p>
<script>
{
...
<div class="maths-by-each">
	<div>
		<table>
			<caption>
				<a href="//float.exposed/0xbe203928"><samp>f</samp> = NUM(−0.15646803)</a><br />
		...
		<table>
			...
			<tr><td colspan=9>NUM(−0.15646803)</td>	<td>∎</td></tr>
		</table>
	</div>
	...
	<div>
		<table>
			<caption>
				<a href="//float.exposed/0x3d3b1da0"><samp>f</samp> = NUM(0.04568255)</a><br />
		...
		<table class="notmath">
			...
			<tr><td colspan=9>NUM(0.04568255)</td>	<td>∎</td></tr>
		</table>
	</div>
</div>
...
<ul>
	<li>MATHJAX(\begin{align}
	            \text{If}\ sgn(f) \text{:}\\<!---->
	            u(f)      &= f \oplus MH(FFFFFFFF) = \neg f\\<!---->
	            u^{-1}(u) &= u \oplus MH(FFFFFFFF) = \neg u\\<!---->
	            \text{Otherwise:}\\<!---->
	            u(f)      &= f \oplus MH(80000000)\\<!---->
	            u^{-1}(u) &= u \oplus MH(80000000)
	            \end{align})</li>
	<li>the sign bit is always flipped, yielding the top bit set for positive NUM(f)s and clear for negative NUM(f)s, inducing a clear partition:<br />
	    NUM(u(<span style="width: 0.7em; display: inline-block;"> </span>0.0)) = <samp>0x8000`0000</samp> = <samp>0b1<small><small>000`0000`0000`0000`0000`0000`0000`0000</small></small></samp><br />
	    NUM(u(<span style="width: 0.7em; display: inline-block;">−</span>0.0)) = <samp>0x7FFF`FFFF</samp> = <samp>0b0<small><small>111`1111`1111`1111`1111`1111`1111`1111</small></small></samp><br />
	    but this means that the condition is mirrored between <samp>u</samp> and <samp>UREV</samp></li>
...
<div class="mathnospace" style="text-align: center;">
<pre>\
u(f)<!--" -->   = asuint(f) ^ (0xFFFF`FFFF if (asuint(f) & 0x8000`0000) else 0x8000`0000)<!--"-->
UREV(u)<!--"--> = asfloat(u ^ (0xFFFF`FFFF if not    (u  & 0x8000`0000) else 0x8000`0000)<!--"-->\
</pre>
</div>

<p class="indented continued">
MATHJAX(B_m) and MATHJAX(B_M)/MATHJAX(B_r) can be allocated directly, minding the free scalars in each 4-primitive block.
...
<div class="codecontinued">
#include "blogn_t/018.21-bbox-shader.h.hlsl.html"
</div>
...
<div class="codecontinued">
#include "blogn_t/018.22-bbox-find_bbox.hlsl.html"
</div>
...
just needs to supply the integral equivalent of MATHJAX((+\infty^3, -\infty^3))
and post-process with <samp>UREV</samp>
(this is where a real enum would've<!--'--> been nice):
</p>
<div class="codecontinued">
#include "blogn_t/018.23-bbox-driver.rs.html"
</div>
...
-->
<p class="indented continued">
This takes anywhere between NUM(120-400µs) on the images in the test dataset on the 5700 XT,
...
more data shows throughput of around NUM(414'419'783 <span class="roboto"><samp>uint3</samp></span>/s).
The next generation
(<a href="//git.sr.ht/~nabijaczleweli/f32-fromto-u32/tree/trunk/item/gpu/NVIDIA GeForce RTX 4070 SUPER 8x.log">NVIDIA RTX 4070 SUPER</a>\
/<a href="//git.sr.ht/~nabijaczleweli/f32-fromto-u32/tree/trunk/item/gpu/AMD Radeon RX 6500 XT (RADV NAVI24) explicit asr forced.log">AMD Radeon RX 6500 XT (RADV NAVI24)</a>)
gets NUM(8'221'135'253<!--'-->)/NUM(8'383'953'229<!--'-->)!
</p>


HEADING(3, grid-grid, he do b hittin it doe)

<p class="indented continued">
With all the parameters to grid MATHJAX(g) figured out, we can compute "which points live in each grid cell" MATHJAX(h).
This is relatively trivial, except the GPU has no structured programming capabilities,
so the natural <samp>uint3 → Vec&lt;indices of known_rgbs/MATHJAX(D)></samp> mapping is inexpressible.
...
(if we force MATHJAX(B_m = (0,\frac{-1}{2},\frac{-1}{2}); B_r = (1,1,1)) and specify MATHJAX(G=10))
</p>

<div class="three-by-each directly continuation">
	   FIG(018.22-grid-alloc-ortho-y.png, "Blown up three-point cloud overlaid with a 10x10 grid",
	       Orthographic projection along MATHJAX(y))<!--
	-->FIG(018.23-grid-alloc-ortho-x.png, "The same, alongside different axis",
	       MATHJAX(x))<!--
	-->FIG(018.24-grid-alloc-ortho-z.png, "Likewise",
	       MATHJAX(z))
</div>

<div class="singles continuation">
	FIG(018.25-grid-alloc-persp.png, "The three-point cloud in 3D overlaid with a 10x10x10 grid",
	    <div class="mathnospace real">
	    BIG_MATHJAX_START
	    \begin{align}
	    g(D_1) &= (7.91765, 6.23666, 3.89675)\\<!---->
	    g(D_2) &= (7.7292,  6.95024, 3.75878)\\<!---->
	    g(D_3) &= (7.5225,  6.89196, 4.36107)
	    \end{align}
	    BIG_MATHJAX_END
	    </div>)
</div>
...
<p class="indented continued">
Thus, the grid cells occupied are, resp. MATHJAX((7, 6, 3)), MATHJAX((7, 6, 3)), and MATHJAX((7, 6, 4)).
The backing storage can then be MATHJAX((1, 2, 3)),
...
(The backing storage could also be MATHJAX((3, 2, 1)), and the map return MATHJAX([2, 4<!--(-->))/MATHJAX([1, 1<!--(-->)).)
</p>
<p class="indented continuing">
Realistically, both the map and backing storage are "a big array" — of
<samp>uint2</samp>s with dimension MATHJAX(G^3) and
<samp>uint</samp>s  with dimension MATHJAX(\overline{Set(D)}) respectively.
It's<!--'--> theoretically possible to implement MATHJAX(h) by collecting the points for each grid cell coordinate separately, like the definition,
but this would be MATHJAX(O(n^2)), so pre-calculating the grid cell coordinates for each point, then collecting them, is MATHJAX(O(n)).
To that end, with MATHJAX(g) as <samp>lab_to_gridpos</samp> and MATHJAX(G) as <samp>GRID_SIZE</samp>,
...
<div class="codecontinued">
#include "blogn_t/018.24-assign_cell-shader.h.hlsl.html"
</div>
<p class="continuing">
...
grows by MATHJAX(G^3 SIZEOF(uint2) = 8\text{MB}), which is good to know but otherwise negligible.
</p>
<p class="indented continuing">
MATHJAX(g)'s<!--'--> MATHJAX(\varepsilon = 0.001) is just as good as any other value (so long as it actually exists around MATHJAX(G));
the optimiser lowers this to a constant.
The pre-computation can thus be MATHJAX(TRUNC \circ g) directly in
...
<div class="codecontinued">
#include "blogn_t/018.25-assign_cell-assign_cell.hlsl.html"
</div>
<p class="continuing">
this <del>ab</del>uses <samp>outputs</samp> as an output buffer —
this is fine, since the size used is MATHJAX(\texttt{known_rgbs.len()} \cdot SIZEOF(uint)),
and the allocation is MATHJAX(2^{24} SIZEOF(float4)),
which is guaranteed to be at least NUM(4×) larger.
Obviously the packing would need to be changed for MATHJAX(G > MH(FF)).
This will not pose an issue.
</p>
<p class="indented continuing">
Shaders can include <samp>shader.h</samp> to have one source of truth for MATHJAX(G), but the Rust driver can't.<!--'-->
This is emulated in
...
<div class="codecontinued">
#include "blogn_t/018.26-assign_cell-build.rs.html"
</div>

<p class="continuing">
So, with, MATHJAX(h) as <samp>groups_data</samp>:
</p>
<div class="codecontinued">
#include "blogn_t/018.27-assign_cell-driver.rs.html"
</div>

<p class="indented continued">
This is the first time more than the aforementioned NUM(4) bindings are used;
...
<p class="indented continuing">
<samp>groups_accumulator</samp> is a similar size to the MATHJAX(8\text{MB}) —
MATHJAX(G^3 \cdot SIZEOF(Vec&lt;_>) = G^3 \cdot 3 SIZEOF(usize) = 24\text{MB}_\text{LP64} \vee 12\text{MB}_\text{ILP32})
...
and it makes no sense to clear it with an actual <samp>memset()</samp>, effectively uploading MATHJAX(8\text{MB}) of zeroes to <samp>DEVICE_LOCAL</samp> memory.
...
(after the memset, the default value is MATHJAX([0, 0 RPAREN = \varnothing) which is what we'd<!--'--> be writing anyway),
...
-->
<table class="perf">
	<tr><th>(colours)       </th> <th>NUM(<!--'-->16'755)</th> <th>NUM(<!--'-->242'931)</th>  <th>NUM(<!--'-->328'666)</th> <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th><samp>assign_cell.hlsl</samp></th><td>NUM(77.0µs)</td>     <td>NUM(225.3µs)</td>          <td>NUM(274.9µs)</td>   <td>NUM(737.3µs)</td> <td>NUM(12µs)/NUM(15µs)/NUM(15µs)/NUM(20µs)</td></tr>
	<tr><th><q>grouping</q> </th>        <td>NUM(5.160ms)</td>        <td>NUM(24.060ms)</td>         <td>NUM(32.644ms)</td>  <td>NUM(94.377ms)</td> <td>NUM(1.339ms)/NUM(1.051ms)/NUM(1.410ms)/NUM(4.375ms)</td></tr>
	<tr><th>grouping speed  </th>        <td>NUM(3246792)</td>        <td>NUM(10097073)</td>         <td>NUM(10068170)</td>  <td>NUM(11690922)</td> <td>[points/s]</td></tr>
	<tr><th><q>copying</q>  </th>        <td>NUM(2.980ms)</td>        <td>NUM(4.717ms)</td>           <td>NUM(5.932ms)</td>   <td>NUM(8.905ms)</td> <td>NUM(564µs)/NUM(315µs)/NUM(404µs)/NUM(500µs)</td></tr>
	<tr><th><samp>GroupDesc</samp>s copied</th><td>NUM(6523)</td>        <td>NUM(66589)</td>           <td>NUM(104645)</td>    <td>NUM(167428)</td> <td>= cells occupied; out of NUM(100<sup>3</sup>)</td></tr>
	<tr><th>…if w/o MATHJAX(B_m)/MATHJAX(B_r) scaling</th><td>NUM(2260)</td><td>NUM(6979)</td>     <td>NUM(10662)</td>     <td>NUM(53492)</td> <td style="text-align: center;">'<!---->'</td></tr>
	<tr><th>memcpy          </th>        <td>NUM(388.2µs)</td>         <td>NUM(436.3µs)</td>          <td>NUM(650.5µs)</td>   <td>NUM(912.4µs)</td> <td>NUM(23µs)/NUM(31µs)/NUM(2µs)/NUM(57µs)</td></tr>
	<tr><th>memcpy size     </th>           <td>NUM(67kB)</td>           <td>NUM(972kB)</td>           <td>NUM(1315kB)</td>    <td>NUM(4413kB)</td></tr>
</table>
<p class="continuing">
So <q>grouping</q>, on the i7-2600, quickly arrives at a consistent NUM(~11Mp/s) throughput.
Bounding box scaling clearly works, increasing the occupancy, in the case of the NUM(1'103'347) image, from NUM(~5%)
(average population of NUM(~20) in non-empty cells, median NUM(9))
to NUM(~17%) (NUM(~7), NUM(3)).
</p>


HEADING_S(3, grid-impl, class="continued", The grid-based mean-shift clustering shader implementation,)
<p class="continuing">
thus, can take advantage of the grid by, instead of checking if <em>all</em> elements of MATHJAX(D) are closer to MATHJAX(q_t) than MATHJAX(r),
...
so the sphere of influence (with a cubic bounding box) turns into an ellipsoid of influence with "radiuses" MATHJAX(R)
(with a prismatic bounding box from MATHJAX(g_{tm}) to MATHJAX(g_{tM}))
rounded up to encompas any cell it intersects
after the MATHJAX(B_r) scaling:
</p>
<div class="mathnospace">
BIG_MATHJAX_START
\begin{align}
...
<div class="singles">
	<figure>
		...
		<figcaption>Effect of the scaling on MATHJAX(r) (as-if MATHJAX(G=1, r=0.2), MATHJAX(R) quantisation not applied)</figcaption>
	</figure>
</div>

<p class="continued">
With
MATHJAX(R)      as <samp>grid_bothdirs</samp>,
MATHJAX(g_t)    as <samp>gridpos</samp>,
MATHJAX(g_{tm}) as <samp>gridpos_from</samp>,
and
MATHJAX(g_{tM}) as <samp>gridpos_to</samp>,
...
can implement the MATHJAX(\bigcup) literally:
</p>
<div class="codecontinued">
#include "blogn_t/018.29-posterise_grid_kernel.hlsl.inc.hlsl.html"
</div>
<p class="continuing">
...
are unchanged,
and so is both the outer-most MATHJAX(q_t) loop and inner-most MATHJAX(\Sigma).
To avoid overflow if MATHJAX(R > g_t) we observe MATHJAX(\max(g_t - R, (0, 0, 0)) = g_t - \min(R, g_t)).
</p>
<p class="indented continuing">
...
allows retaining the same size while fitting <samp>grid_bothdirs</samp>/MATHJAX(R):
</p>
<div class="codecontinued">
#include "blogn_t/018.30-posterise_grid-shader.h.hlsl.html"
</div>
<p class="indented continuing">
MATHJAX(R) computation is can likewise be
...
<div class="codecontinued">
#include "blogn_t/018.31-posterise_grid-driver.rs.html"
</div>
...
(additionally assuming every cell inspected will be non-empty, which it won't<!--'-->,
 but non-empty cells are where the points are so they're<!--'--> roughly where MATHJAX(q_t)s are
 and they're<!--'--> pretty dense, so.):
</p>
<table class="perf">
	<caption>MATHJAX(r=0.02, G=100)</caption>
	<tr><th>(colours)</th>                                                       <th>NUM(<!--'-->16'755)</th> <th>NUM(<!--'-->242'931)</th> <th>NUM(<!--'-->328'666)</th> <th>NUM(1'103'347)</th></tr>
	<tr><th>MATHJAX(R)</th>                                                               <td>NUM(3×7×8)</td>           <td>NUM(3×1×7)</td>          <td>NUM(3×8×10)</td>     <td>NUM(2×5×4)</td></tr>
	<tr><th>Volume between MATHJAX(g_{tm}) and MATHJAX(g_{tM}) when not near an edge</th> <td>NUM( 1785)</td>           <td>NUM(  315)</td>          <td>NUM(  2499)</td>     <td>NUM(  495)</td></tr>
	<tr><th>Average points inspected when not near an edge</th>                           <td>NUM( 4585)</td>           <td>NUM( 1149)</td>          <td>NUM(  7849)</td>     <td>NUM( 3262)</td></tr>
	<tr><th>Projected average speed-up</th>                                               <td>NUM(72.64%)</td>          <td>NUM(99.53%)</td>          <td>NUM(97.61%)</td>    <td>NUM(99.70%)</td></tr>
</table>
...
<table class="perf">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(116.646ms)</td>               <td>NUM( 99.415ms)</td>               <td>NUM( 85.23%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(    5.49%)</td>               <td>NUM(   12.35%)</td>               <td>NUM( -6.68 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(    9.47%)</td>               <td>NUM(   16.36%)</td>               <td>NUM( -7.02 p.p.)</td></tr>
	<tr></tr>
	<tr><th>(colours)              </th> <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(115.426ms)</td>               <td>NUM(144.812ms)</td>               <td>NUM(125.46%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(    6.75%)</td>               <td>NUM(    1.54%)</td>               <td>NUM(  6.63 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(   11.87%)</td>               <td>NUM(    4.64%)</td>               <td>NUM(  9.51 p.p.)</td></tr>
</table>
<p class="continuation">
MULFREQ becomes more slower than PALETTE in the degenerate case and less slower than PALETTE on the real image.
What this means is unclear to me.
</p>

<table class="perf left">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>       <td>NUM(116.65ms)</td>          <td>NUM(4.198s)</td>    <td>TIMEOUT(109, 155)</td> <td>NUM(5ms)/NUM(214ms)/–</td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(     10%)</td>            <td>NUM( 36%)</td>          <td>NUM(    4%)</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM( 99.42ms)</td>          <td>NUM(3.286s)</td>    <td>TIMEOUT(190, 152)</td> <td>NUM(782µs)/NUM(332ms)/–</td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(     18%)</td>            <td>NUM( 25%)</td>          <td>NUM(   31%)</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>         <td>NUM(1.891s)</td>     <td>TIMEOUT(73, 220)</td>  <td>TIMEOUT(9560, 6505)</td> <td>NUM(400ms)/–/–</td></tr>
	<tr><th>(speed-up vs base-line)</th>           <td>NUM(-30%)</td>         <td>NUM(      ∞)</td>          <td>NUM(42.58×)</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM(567.45ms)</td>    <td>TIMEOUT(143, 132)</td>  <td>TIMEOUT(7482, 5075)</td> <td>NUM(50ms)/–/–</td></tr>
	<tr><th>(speed-up vs base-line)</th>           <td>NUM(-52%)</td>            <td>NUM( 81%)</td>            <td>NUM(4.6×)</td> <td></td></tr>
	TIMEOUT_PERF_TFOOT(4)
</table>
...
<div class="singles">
FIG(018.27-graphml-gridded.png, "", )
</div>


HEADING(2, cicada-grid, Failure: GPU-side grid-cell-to-index-list mapping)

<p class="indented continued">
While the collection (<q>grouping</q>) can't<!--'--> be sped up on the host, it could, theoretically,
be executed on the GPU by collecting each value of MATHJAX(h) directly from the definition by looking at each element of MATHJAX(D) for each cell,
...
<div class="codecontinued">
#include "blogn_t/018.32-cicada_grid-assign_cell.hlsl.html"
</div>
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.33-cicada_grid-shader.h.hlsl.html"
</div>
...
<div class="codecontinued">
#include "blogn_t/018.34-cicada_grid-driver.rs.html"
</div>
<p class="indented continued">
The initial offset must be initialised (to NUM(1) instead of NUM(0) to not overwrite itself),
but this can be done in the same command buffer as dispatching the shader and the "couple dozen μs" (like NUM(79±32) in this case) become free.
...
is dispatched at MATHJAX(\text{numthreads} = (G, 1, 1), \text{dispatch}(1, G, G)) to evaluate every integer in MATHJAX(h)'s<!--'--> domain of MATHJAX([0, G RPAREN^3):
</p>
<div class="codecontinued">
#include "blogn_t/018.35-cicada_grid-cicada_grid.hlsl.html"
</div>
...
-->
<p class="continued">
Visiting <em>every</em> point in MATHJAX(Set(D)) for <em>every</em> occupied cell
(up to MATHJAX(G^3) in theory but cf. occupancy rates above; there are MATHJAX(G^3) (so, NUM(1'000'000)), threads dispatched though),
is, expectedly, ruinous:
</p>
<table class="perf">
	<tr><th>(colours)                                </th> <th>NUM(<!--'-->16'755)</th> <th>NUM(<!--'-->242'931)</th>  <th>NUM(<!--'-->328'666)</th> <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th><samp>assign_cell.hlsl</samp>            </th>         <td>NUM(96.5µs)</td>         <td>NUM(113.9µs)</td>            <td>NUM(115µs)</td>   <td>NUM(156.8µs)</td> <td>NUM(23µs)/NUM(22µs)/NUM(22µs)/NUM(13µs)</td></tr>
	<tr><th>speed-up vs previous                     </th>        <td>NUM(-25.27%)</td>         <td>NUM( 49.44%)</td>          <td>NUM( 58.17%)</td>   <td>NUM( 78.73%)</td> <td></td></tr>

	<tr><th><q>grouping</q> + <q>copying</q> + memcpy</th>        <td>NUM(5.898ms)</td>       <td>NUM(181.893ms)</td>        <td>NUM(322.803ms)</td><td>NUM(1396.184ms)</td> <td>NUM(56µs)/NUM(374µs)/NUM(430µs)/NUM(815µs)</td></tr>
	<tr><th>speed-up vs previous                     </th>        <td>NUM( 32.91%)</td>  <td>ONEOVER(6.24)NUM(×)</td>   <td>ONEOVER(8.27)NUM(×)</td><td>ONEOVER(13.4)NUM(×)</td> <td></td></tr>
</table>
<p class="continuing">
...
<p class="indented continued">
Even another NUM(40×) speed-up wouldn't<!--'--> guarantee we can compute every MATHJAX(f) within the NUM(~8s) driver-imposed limit.
But, each shader call could iterate MATHJAX(t), like, NUM(10) times.
The points that converge can be committed as the final MATHJAX(f) mapping as previous,
but the ones that don't<!--'--> can be saved and
</p>
HEADING_S(2, oneshot, class="continuing", processed piece-meal.)
<p class="indented continuing">
...
<div class="codecontinued">
#include "blogn_t/018.36-oneshot-posterise_kernel.hlsl.inc.hlsl.html"
</div>

<p class="indented continued">
Since the amount of unconverged points decreases each iteration, the MATHJAX(q_t) points-to-process array must be compacted between iterations.
...
where it may take NUM(64) pregnant threads where NUM(63) have few fast-to-solve points and NUM(1) that takes long to converge,
as long as the latter to bear an output
(this is hopefully attenuated slightly by the points being sorted by MATHJAX(RRGGBB16) so similar colours in similar natal situations are carried together, but.).
We can store MATHJAX(q_t)s and MATHJAX(q_{t+10})s in alternating ranges of a big array and page-flip the input/output iterators
(this makes initialisation a memcpy from <samp>known_rgbs_bundle</samp> as MATHJAX(q_0), which is quite nice from a point-and-woo-at-definition perspective):
</p>
<div class="singles continuation">
	FIG(018.28-current_rgbs_bundle.png, "Data flow diagram; known_rgbs_bundle (Set(D)) is copied to the start of current_rgbs_bundle, double the length of Set(D), and this is q_0; this then is shorter and copied to the second half of current_rgbs_bundle as q_10; this is then shorter and copied to the front half of current_rgbs_bundle again as q_20" class="dark-invert", )
</div>
<p class="indented continuing">
...
so in this case we want to index <samp>known_rgbs_freqs</samp> by MATHJAX(RRGGBB16) (like <samp>outputs</samp>),
which is kept with the state.
</p>
<div class="codecontinued">
#include "blogn_t/018.37-oneshot-driver.rs.html"
</div>
<p class="continuation">
The actual value of MATHJAX(10) is roughly tuned to dispatch shaders that don't make effectively-zero progress and also don't take NUM(>8s)
...
NUM(>50'000<!--'-->) is as good of a threshold as any.
(The corresponding
...
<div class="codecontinued">
#include "blogn_t/018.38-oneshot-posterise_grid_oneshot.kernel.hlsl.inc.hlsl.html"
</div>
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.39-oneshot-posterise_grid_oneshot.hlsl.html"
</div>
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.40-oneshot-posterise_grid_oneshot.mul.hlsl.html"
</div>
<p class="continuing">
The expansion for PALETTE is entirely unchanged:
</p>
<div class="codecontinued">
#include "blogn_t/018.41-oneshot-posterise_grid_oneshot-inlined.hlsl.html"
</div>
<p class="continuing">
But MULFREQ reads more data more indirectly:
</p>
<div class="codecontinued">
#include "blogn_t/018.42-oneshot-posterise_grid_oneshot.mul-inlined.hlsl.html"
</div>
...
-->
#define AT \operatorname{@}
<table class="perf ">
	<caption><code>use_oneshot_driver = true</code></caption>
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th> <th>±σ</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(134.983ms)</td>               <td>NUM(142.337ms)</td>               <td>NUM(105.45%)</td>      <td>NUM(6.6ms)/NUM(6.8ms)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(  -15.72%)</td>               <td>NUM(  -43.17%)</td>               <td>NUM( 20.22 p.p.)</td>  <td></td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(   -4.76%)</td>               <td>NUM(  -19.75%)</td>               <td>NUM( 13.20 p.p.)</td>  <td></td></tr>
	<tr><th>iterations MATHJAX((pointsdone AT depth))</th> <td>NUM(2) MATHJAX(((8726 AT q_{20}), (8029 AT \infty)))</td> <td>NUM(2) MATHJAX(((9892 AT q_{20}), (6863 AT \infty)))</td> <td></td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th> <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th> <th>±σ</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(136.594ms)</td>               <td>NUM(202.383ms)</td>               <td>NUM(148.16%)</td>      <td>NUM(6.6ms)/NUM(10.4ms)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(  -18.34%)</td>               <td>NUM(  -39.76%)</td>               <td>NUM( 22.71 p.p.)</td>  <td></td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(   -4.29%)</td>               <td>NUM(  -33.27%)</td>               <td>NUM( 32.21 p.p.)</td>  <td></td></tr>
	<tr><th>iterations MATHJAX((pointsdone AT depth))</th> <td colspan=2 style="text-align: center;">NUM(2) MATHJAX(((8726 AT q_{20}), (8029 AT \infty)))</td> <td></td> <td></td></tr>
</table>
<p class="continuing">
...
As ever, DUM() is equivalent except for measuring the relative cost of MULFREQ:
the indirections really show — this is the first time that MULFREQ is <em>slower</em> than PALETTE in the non-degenerate case.
</p>
<p class="indented continuation">
...
-->
#define ITERSUMDETR(s, ...) <details><summary>s</summary> MATHJAX(__VA_ARGS__)</details>
#define ITERSUMDET(n, ...) ITERSUMDETR(NUM(n), __VA_ARGS__)
#define SLOWDOWN_DOESNT <strong>*</strong>: slow-down doesn't<!--'--> apply (and subsequent measurement comparisons will ignore it)<br />
#define SLOWDOWN_DIDNT_GENERIC(...) <tfoot class="slowdown-didnt"><tr><td colspan=5>__VA_ARGS__</td></tfoot>
#define SLOWDOWN_DIDNT(...) SLOWDOWN_DIDNT_GENERIC(__VA_ARGS__ <strong>—</strong>: didn't<!--'--> complete before)
<table class="perf left" id="itersumdet">
	<caption><code>use_oneshot_driver = true</code></caption>
	<tr><th>(colours)              </th><th>*NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>      <td>NUM(134.983ms)</td>          <td>NUM(6.090s)</td>        <td>NUM(110.051s)</td> <td>NUM(6.6ms)/NUM(21.5ms)/NUM(1.958s)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM( -15.72%)</td>        <td>NUM( -45.07%)</td>                    <td>—</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  -4.37%)</td>         <td>NUM(  7.82%)</td>                    <td>—</td> <td></td></tr>
	<tr><th>ITERSUMDETR(iterations, (pointsdone AT depth))</th> <td>ITERSUMDET(2, (8726 AT q_{20}), (8029 AT \infty))</td> <td>ITERSUMDET(20, (3439 AT q_{20}), (4585 AT q_{40}), (4990 AT q_{60}), (7237 AT q_{80}), (8241 AT q_{100}), (1893 AT q_{120}), (1537 AT q_{140}), (1434 AT q_{160}), (1165 AT q_{180}), (1606 AT q_{200}), (1593 AT q_{220}), (2736 AT q_{240}), (4634 AT q_{260}), (7876 AT q_{280}), (7448 AT q_{300}), (4920 AT q_{320}), (4170 AT q_{340}), (3074 AT q_{360}), (2309 AT q_{380}), (9973 AT \infty))</td> <td>ITERSUMDET(63, (409 AT q_{10}), (536 AT q_{20}), (489 AT q_{30}), (481 AT q_{40}), (544 AT q_{50}), (678 AT q_{60}), (907 AT q_{70}), (1251 AT q_{80}), (1929 AT q_{90}), (2851 AT q_{100}), (4193 AT q_{110}), (5704 AT q_{120}), (4411 AT q_{130}), (2533 AT q_{140}), (2646 AT q_{150}), (2672 AT q_{160}), (2617 AT q_{170}), (2507 AT q_{180}), (2516 AT q_{190}), (2675 AT q_{200}), (2591 AT q_{210}), (2588 AT q_{220}), (2597 AT q_{230}), (2768 AT q_{240}), (2961 AT q_{250}), (3020 AT q_{260}), (3059 AT q_{270}), (3049 AT q_{280}), (3090 AT q_{290}), (3296 AT q_{300}), (3187 AT q_{310}), (3256 AT q_{320}), (3291 AT q_{330}), (3275 AT q_{340}), (3615 AT q_{350}), (3757 AT q_{360}), (3871 AT q_{370}), (3849 AT q_{380}), (3836 AT q_{390}), (3705 AT q_{400}), (3818 AT q_{410}), (3911 AT q_{420}), (4062 AT q_{430}), (4193 AT q_{440}), (4413 AT q_{450}), (4413 AT q_{460}), (4387 AT q_{470}), (4500 AT q_{480}), (4547 AT q_{490}), (9053 AT q_{510}), (9153 AT q_{530}), (9378 AT q_{550}), (8867 AT q_{570}), (8828 AT q_{590}), (8023 AT q_{610}), (7043 AT q_{630}), (6459 AT q_{650}), (5885 AT q_{670}), (5115 AT q_{690}), (4191 AT q_{710}), (3501 AT q_{730}), (2921 AT q_{750}), (9060 AT \infty))</td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM(142.337ms)</td>          <td>NUM(6.687s)</td>         <td>NUM(53.222s)</td> <td>NUM(6.8ms)/NUM(59.9ms)/NUM(1.702s)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM( -43.17%)</td>        <td>NUM(-103.48%)</td>                    <td>—</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM( -17.53%)</td>        <td>NUM( -53.49%)</td>                    <td>—</td> <td></td></tr>
	<tr><th>ITERSUMDETR(iterations, (pointsdone AT depth))</th> <td>ITERSUMDET(2, (9892 AT q_{20}), (6863 AT \infty))</td> <td>ITERSUMDET(12, ( 4698 AT q_{20}), ( 6368 AT q_{40}), (11911 AT q_{60}), (12558 AT q_{80}), ( 7092 AT q_{100}), (11538 AT q_{120}), ( 8300 AT q_{140}), ( 5246 AT q_{160}), ( 3911 AT q_{180}), ( 2755 AT q_{200}), ( 2384 AT q_{220}), ( 8099 AT \infty))</td> <td>ITERSUMDET(23, (1853 AT q_{10}), (11458 AT q_{20}), (8918 AT q_{30}), (7158 AT q_{40}), (6140 AT q_{50}), (7593 AT q_{60}), (10377 AT q_{70}), (13072 AT q_{80}), (15944 AT q_{90}), (18398 AT q_{100}), (16329 AT q_{110}), (16083 AT q_{120}), (15680 AT q_{130}), (23235 AT q_{150}), (15649 AT q_{170}), (11954 AT q_{190}), (9276 AT q_{210}), (6745 AT q_{230}), (5624 AT q_{250}), (4809 AT q_{270}), (4091 AT q_{290}), (2944 AT q_{310}), (9601 AT \infty))</td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th><th>*NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>         <td>NUM(2.773s)</td>        <td>NUM(123.019s)</td>         <td>NUM(49.478s)</td> <td>NUM(454.9ms)/NUM(1.77s)/NUM(973ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM( -46.61%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM( -90.46%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	<tr><th>ITERSUMDETR(iterations, (pointsdone AT depth))</th> <td>ITERSUMDET(10, ( 1435 AT q_{20}), ( 2011 AT q_{40}), ( 3731 AT q_{60}), ( 3481 AT q_{80}), ( 2988 AT q_{100}), ( 2211 AT q_{120}), (  589 AT q_{140}), (  629 AT q_{160}), (  805 AT q_{180}), ( 9988 AT \infty))</td> <td>ITERSUMDET(32, (2863 AT q_{10}), (2399 AT q_{20}), (4471 AT q_{30}), (6288 AT q_{40}), (10097 AT q_{50}), (14279 AT q_{60}), (15496 AT q_{70}), (19285 AT q_{80}), (17463 AT q_{90}), (11674 AT q_{100}), (10639 AT q_{110}), (10127 AT q_{120}), (11113 AT q_{130}), (13380 AT q_{140}), (15469 AT q_{150}), (16069 AT q_{160}), (14800 AT q_{170}), (11883 AT q_{180}), (9652 AT q_{190}), (8774 AT q_{200}), (8787 AT q_{210}), (17262 AT q_{230}), (13791 AT q_{250}), (12693 AT q_{270}), (11791 AT q_{290}), (9938 AT q_{310}), (7493 AT q_{330}), (4933 AT q_{350}), (3457 AT q_{370}), (2139 AT q_{390}), (1250 AT q_{410}), (8911 AT \infty))</td> <td>ITERSUMDET(16, (12380 AT q_{10}), (80853 AT q_{20}), (174244 AT q_{30}), (186416 AT q_{40}), (151358 AT q_{50}), (118824 AT q_{60}), (87645 AT q_{70}), (72871 AT q_{80}), (56003 AT q_{90}), (38613 AT q_{100}), (31674 AT q_{110}), (38589 AT q_{130}), (22957 AT q_{150}), (12058 AT q_{170}), (8975 AT q_{190}), (9887 AT \infty))</td></tr>
	<tr><th>5700 XT, MULFREQ       </th>      <td>NUM(746.454ms)</td>         <td>NUM(77.749s)</td>         <td>NUM(24.291s)</td> <td>NUM(17.3ms)/NUM(829ms)/NUM(1.062s)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM( -31.55%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM( -99.89%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	<tr><th>ITERSUMDETR(iterations, (pointsdone AT depth))</th> <td>ITERSUMDET( 3, (17216 AT q_{20}), ( 7467 AT q_{40}), ( 3185 AT \infty))</td> <td>ITERSUMDET(63, (317 AT q_{10}), (362 AT q_{20}), (878 AT q_{30}), (1789 AT q_{40}), (2419 AT q_{50}), (3814 AT q_{60}), (3577 AT q_{70}), (2930 AT q_{80}), (3392 AT q_{90}), (3645 AT q_{100}), (3472 AT q_{110}), (4022 AT q_{120}), (4420 AT q_{130}), (4944 AT q_{140}), (6067 AT q_{150}), (8302 AT q_{160}), (6410 AT q_{170}), (4369 AT q_{180}), (4688 AT q_{190}), (4601 AT q_{200}), (4590 AT q_{210}), (4849 AT q_{220}), (4877 AT q_{230}), (4993 AT q_{240}), (5327 AT q_{250}), (5790 AT q_{260}), (6589 AT q_{270}), (7088 AT q_{280}), (7359 AT q_{290}), (7199 AT q_{300}), (6704 AT q_{310}), (6672 AT q_{320}), (6604 AT q_{330}), (6554 AT q_{340}), (6509 AT q_{350}), (6431 AT q_{360}), (6327 AT q_{370}), (5989 AT q_{380}), (5642 AT q_{390}), (5447 AT q_{400}), (5105 AT q_{410}), (4979 AT q_{420}), (4806 AT q_{430}), (4710 AT q_{440}), (4574 AT q_{450}), (4581 AT q_{460}), (4327 AT q_{470}), (8770 AT q_{490}), (8484 AT q_{510}), (8201 AT q_{530}), (7705 AT q_{550}), (7465 AT q_{570}), (7143 AT q_{590}), (6636 AT q_{610}), (6336 AT q_{630}), (5917 AT q_{650}), (5208 AT q_{670}), (4925 AT q_{690}), (4257 AT q_{710}), (3813 AT q_{730}), (3644 AT q_{750}), (2455 AT q_{770}), (8667 AT \infty))</td> <td>ITERSUMDET( 7, (269906 AT q_{10}), (475001 AT q_{20}), (202825 AT q_{30}), (82171 AT q_{40}), (50097 AT q_{60}), (16122 AT q_{80}), (7225 AT \infty))</td></tr>
	SLOWDOWN_DIDNT(SLOWDOWN_DOESNT)
</table>
<script>
...
Images with many complete for the first time, and this explains why NUM(<!--'-->328'666) is so much slower than NUM(1'103'347) —
a maximum observed iteration depth of MATHJAX(q_{410})/MATHJAX(q_{770}) beats MATHJAX(q_{190})/MATHJAX(q_{80}).
</p>
...
<div class="side-by-each continuation">
	FIG(018.29-oneshot-palette.png, "Plot of current_rgbs_len against t for palette" class="dark-invert",
	    (final MATHJAX(\infty) step rendered as-if it were NUM(10) long))
	FIG(018.30-oneshot-mulfreq.png, "Plot of current_rgbs_len against t for *freq"   class="dark-invert",
	    (likewise))
</div>


HEADING(2, expand_rgb, straight up "bussing it" and by "it", haha, well. let's<!--'--> justr say. My rgbs.)

<p class="indented continued">
It's convenient to replace the CPU implementation isocallingconventionally but it's also a little silly and goofy
to run the MATHJAX(p)/MATHJAX(p'<!--'-->) transformations on the CPU when a dedicated FPU is available.
<samp>known_rgbs</samp> work items <a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/8207e447aa91db490f70de0b9f1052c3e949c29f/item/src/main.rs#L160">
	can be thinned</a> to just MATHJAX((RRGGBB16, \text{multiplicity of}\ rC,gC,bC)),
and the returned <samp>outputs</samp> MATHJAX(f) mapping reduced from MATHJAX(C \to P) directly to MATHJAX(C \to C),
so the whole <a href="#RgbStuffedInLab"><samp>RgbStuffedInLab</samp></a> dance can likewise go the way of the dinosaur:
</p>
<div class="codecontinued">
#include "blogn_t/018.43-expand_rgb-main.rs.html"
</div>
<p class="indented continuing">
...
<div class="codecontinued">
#include "blogn_t/018.44-expand_rgb-driver.rs.html"
</div>
...
is a trivial MATHJAX(p \circ I):
</p>
<div class="codecontinued">
#include "blogn_t/018.45-expand_rgb-expand_rgb.hlsl.html"
</div>
<p class="continuing">
MATHJAX(p^{-1}) distributed across the kernels:
</p>
<div class="codecontinued">
#define FORCELINK(f, l) <a href=STR(httpsCOLON_SLASH_SLASHgit.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/8207e447aa91db490f70de0b9f1052c3e949c29f/item/src/f##HASH##L##l)>f</a>
#include "blogn_t/018.46-expand_rgb-kernels.hlsl.html"
#undef FORCELINK
</div>
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.47-expand_rgb-shader.h.hlsl.html"
</div>
...
-->
<table class="perf">
	<tr><th>(colours)                                         </th> <th>NUM(<!--'-->16'755)</th> <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th><samp>known_rgbs</samp> before                    </th>      <td>NUM( 14.680ms)</td> <td>NUM(210.771ms)</td> <td>NUM(339µs)/NUM(21.17ms)</td></tr>
	<tr><th><samp>dest_lab = KnownRgbsPack { rgb, lab }</samp></th>      <td>NUM( 72.793µs)</td> <td>NUM(  5.397ms)</td> <td>NUM(7.87µs)/NUM(168.2µs)</td></tr>
	<tr class="spc"></tr>
	<tr><th><samp>known_rgbs</samp> after                     </th>      <td>NUM( 12.115ms)</td> <td>NUM( 28.952ms)</td> <td>NUM(1.04ms)/NUM(848µs)</td></tr>
	<tr><th><samp>bundle.rgb = known</samp>                   </th>      <td>NUM(141.841µs)</td> <td>NUM( 10.076ms)</td> <td>NUM(11.67µs)/NUM(237.5µs)</td></tr>
	<tr><th><samp>expand_rgb.hlsl</samp>                      </th>      <td>NUM(129.086µs)</td> <td>NUM(200.058µs)</td> <td>NUM(621µs)/NUM(452µs)</td></tr>
	<tr><th>(speed-up)                                        </th>      <td>NUM(   16.04%)</td> <td>NUM(   81.85%)</td> <td></td></tr>
</table>
<p class="continuing">
this is fixed analysis cost for an image
(and, interestingly, a quarter of the bytes in <samp>known_rgbs_bundle</samp> is NUM(~2×) slower;
...
<samp>RgbStuffedInLab</samp> is considered to be distributed across each final MATHJAX(f) computation on the GPU, thus becoming immaterial:
</p>
<!--
...
-->
<table class="perf ">
	<tr><th>(colours)                                           </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(1'103'347), PALETTE</th> <th>±σ</th></tr>
	<tr><th><samp>outputs[…].l = buffer_outputs_map[…][0]</samp></th>      <td>NUM(  2.471ms)</td>                   <td>NUM(162.519ms)</td> <td>NUM(303µs)/NUM(234µs)</td></tr>
	<tr><th><samp>RgbStuffedInLab</samp>                        </th>      <td>NUM(  1.405ms)</td>                    <td>NUM(53.867ms)</td> <td>NUM(57.66µs)/NUM(797.24ms)</td></tr>
	<tr class="spc"></tr>
	<tr><th><samp>outputs[…][0] = out >> 16</samp>              </th>      <td>NUM( 1.2913ms)</td>                   <td>NUM( 82.727ms)</td> <td>NUM(4.49ms)/NUM(2.12ms)</td></tr>
	<tr><th>(speed-up)                                          </th>      <td>NUM(   66.69%)</td>                   <td>NUM(   61.77%)</td> <td></td></tr>
</table>
<p class="continuation">
For NUM(<!--'-->16'755) this is NUM(~16% → ~12%) of overall runtime; for NUM(1'103'347) — NUM(~0.75% → ~0.22%).
Not meaningless.
</p>

<div class="singles">
FIG(018.31-graphml-expand_rgb.png, "", )
</div>
...
HEADING_S(2, GAAP, class="continuing", our very own Enron scandal.)
...
<div class="codecontinued">
#include "blogn_t/018.48-GAAP-main.rs.html"
</div>
...
<div class="codecontinued">
#include "blogn_t/018.49-GAAP-driver.rs.html"
</div>
<p class="continuing">
...
<table class="perf">
	<tr><th>(colours)                           </th> <th>NUM(<!--'-->16'755)</th> <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th><samp>known_rgbs_{rgbs,freqs}</samp></th>      <td>NUM( 14.675ms)</td> <td>NUM( 27.954ms)</td> <td>NUM(1.04ms)/NUM(1.92ms)</td></tr>
	<tr><th>(speed-up)                          </th>      <td>NUM(  -21.13%)</td> <td>NUM(    3.45%)</td> <td></td></tr>
</table>
<p class="continuing">
this is, practically, a wash (σ is like NUM(6%)); overall re-accounting like this remits
</p>
<table class="perf">
	<tr><th>(colours)                      </th><th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, <samp>measure()</samp></th>     <td>NUM(  8.095ms)</td>       <td>NUM( 17.478ms)</td>       <td>NUM( 33.629ms)</td> <td>NUM(918µs)/NUM(4.27ms)/NUM(2.58ms)</td></tr>
	<tr><th>(of previous, PALETTE)         </th>     <td>NUM(  7.01%)</td>         <td>NUM(  0.29%)</td>         <td>NUM(  0.03%)</td> <td></td></tr>
	<tr><th>(of previous, MULFREQ)         </th>     <td>NUM(  5.59%)</td>         <td>NUM(  0.26%)</td>         <td>NUM(  0.06%)</td> <td></td></tr>
	<tr></tr>
	<tr><th>(colours)                      </th><th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, <samp>measure()</samp></th>     <td>NUM(  9.298ms)</td>       <td>NUM( 45.637ms)</td>       <td>NUM(140.834ms)</td> <td>NUM(948µs)/NUM(5ms)/NUM(24.82ms)</td></tr>
	<tr><th>(of previous, PALETTE)         </th>       <td>NUM(  0.49%)</td>         <td>NUM(  0.04%)</td>         <td>NUM(  0.28%)</td> <td></td></tr>
	<tr><th>(of previous, MULFREQ)         </th>       <td>NUM(  1.64%)</td>         <td>NUM(  0.06%)</td>         <td>NUM(  0.58%)</td> <td></td></tr>
</table>
<p class="continuing">
(though, of course, the PALETTE path now also copies frequency data for NUM(40µs-20ms) which it didn't<!--'--> before).
</p>

<div class="singles">
FIG(018.32-graphml-GAAP.png, "", )
</div>


HEADING_S(2, known_rgbs-DEVICE_LOCAL, class="continued", Do NOT tell americans about your allocations. Worst mistake of my life.)
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.50-known_rgbs-DEVICE_LOCAL-driver.rs.html"
</div>
<p class="indented continuing">
...
<div class="codecontinued">
#include "blogn_t/018.51-known_rgbs-DEVICE_LOCAL-expand_rgb.hlsl.html"
</div>
<p class="continuing">
and the new
...
<div class="codecontinued">
#include "blogn_t/018.52-known_rgbs-DEVICE_LOCAL-distribute_oneshot_freqs.hlsl.html"
</div>
...
<table class="perf left">
	<tr><th>(colours)                                           </th> <th>NUM(<!--'-->16'755)</th> <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>host → <samp>known_rgbs_{rgbs,freqs}</samp>         </th>      <td>NUM(    226µs)</td>  <td>NUM(29.529ms)</td> <td>NUM(124µs)/NUM(62.8µs)</td></tr>
	<tr><th>(proportion of <samp>measure()</samp>)              </th>           <td>NUM(2.8%)</td>       <td>NUM(21%)</td> <td></td></tr>
	<tr><th>memcpy or <samp>distribute_oneshot_freqs.hlsl</samp></th>      <td>NUM(    130µs)</td>     <td>NUM(770µs)</td> <td>NUM(62.8µs)/NUM(21µs)</td></tr>
	<tr><th>(speed-up, 5700 XT)                                 </th>      <td>NUM(   42.72%)</td> <td>NUM(  38.347×)</td> <td></td></tr>
	<tr><th>(proportion of <samp>measure()</samp>)              </th>           <td>NUM(1.6%)</td>      <td>NUM(0.6%)</td> <td></td></tr>
</table>

<p class="indented">
This leaves just <samp>params</samp> (constant MATHJAX(G^3 SIZEOF((uint, uint)) + \varepsilon \approx 8\text{MB} \approx 3\%)) in <code>DEVICE_LOCAL & HOST_VISIBLE</code>,
freeing MATHJAX(\overline{Set(I)}(SIZEOF(struct known_rgbs_pack) + SIZEOF(uint)) = \overline{Set(I)} \cdot 20\text{B})
(MATHJAX(\in [335\text{kB}, 22\text{MB}] \approx [0.13\%, 8.6\%]) in test data;
 theoretical maximum MATHJAX(320\text{MiB} = 125\%)).
</p>

<div class="singles">
FIG(018.33-graphml-known_rgbs-DEVICE_LOCAL.png, "", )
</div>


HEADING(2, gilbert, You do not under any circumstances have to hand it to Scott Adams)

<p class="indented continued">
The <samp>[start, end)</samp> pairs in the <samp>groups_data</samp> half of MATHJAX(h) are <a href="#grid-grid">laid out</a> in natural axis order,
...
<!-- MATHJAX(\forall x,y,z \in [0, G RPAREN \cap \mathbb{Z} : \texttt{group_ranges}[x][y][z] = G^2x + Gy + z) -->
MATHJAX(\forall x,y,z \in \bar{G} : \texttt{group_ranges}[x][y][z] = G^2x + Gy + z)
and cast the traversal order back onto the the grid thusly
</p>
<div class="singles continuing">
	FIG(018.34-gilbert-iterorder+default.png, "Point cloud ε overlaid with a section of a 20x20x20 grid, and highlighted straight lines through center of each grid cell across each axis",
	    Effective look-up order (starts bottom-right) for image <samp EXAMPLE_Ε_STYLE>ε</samp> with MATHJAX(G=20, r=0.05) around MATHJAX((13, 7, 16)))
</div>
<p class="continuing">
...
The ideal iteration order MATHJAX(l) would, in all possible cases, maximally preserve locality
(such that geometrically close cells'<!--'--> data is memorically close in <samp>groups_data</samp>):
<!-- MATHJAX(\forall x,y,z \in [0, G RPAREN \cap \mathbb{Z} : \texttt{group_ranges}[x][y][z] = l(G^2x + Gy + z)). -->
MATHJAX(\forall x,y,z \in \bar{G} : \texttt{group_ranges}[x][y][z] = l(G^2x + Gy + z)).
</p>
<p class="indented continuing">
...
Unfortunately, Hilbert curves are only defined for square/cubic domains with side length MATHJAX(2^n), which MATHJAX(G=100) is not.
Maybe MATHJAX(G \in \{64, 128\}) could work:
</p>
<script>
{
...
<div class="codecontinued">
#define decodeh <a href="//paste.sr.ht/~nabijaczleweli/86c046d7743aab3af2044f5f1b4b30980bb7be01">decodeh</a>
#include "blogn_t/018.53-gilbert-hilbert-driver.rs.html"
#undef decodeh
</div>
...
may, after <a href="//github.com/jakubcerveny/gilbert/pull/17">a quick port</a>, allow using a more optimal MATHJAX(G):
</p>

<table class="perf" id="gilbert-straight">
	<tr><th colspan=2>(colours)                   </th> <th>NUM(<!--'-->16'755)</th> <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th></th></tr>
	<tr><th>straight</th> <th>NUM( 64<sup>3</sup>)</th>       <td>NUM( 94.98ms)</td>        <td>NUM(  6.01s)</td>       <td>NUM(   97.67s)</td> <td></td></tr>
	<tr><th>straight</th> <th>NUM( 80<sup>3</sup>)</th>       <td>NUM(128.99ms)</td>        <td>NUM(  6.87s)</td>       <td>NUM(   83.75s)</td> <td></td></tr>
	<tr><th>straight</th> <th>NUM( 90<sup>3</sup>)</th>       <td>NUM(126.06ms)</td>        <td>NUM(  6.98s)</td>       <td>NUM(  128.76s)</td> <td></td></tr>
	<tr><th>straight</th> <th>NUM(100<sup>3</sup>)</th>       <td>NUM(120.33ms)</td>        <td>NUM(  6.81s)</td>       <td>NUM(  119.00s)</td> <td></td></tr>
	<tr><th>straight</th> <th>NUM(128<sup>3</sup>)</th>       <td>NUM(162.10ms)</td>        <td>NUM( 10.74s)</td>       <td>NUM(  129.18s)</td> <td></td></tr>
	<tr class="hspc"></tr>
	<tr><th>gilbert </th> <th>NUM( 80<sup>3</sup>)</th>       <td>NUM(130.03ms)</td>        <td>NUM(  6.80s)</td>       <td>NUM(   96.04s)</td> <td></td></tr>
	<tr><th>gilbert </th> <th>NUM( 90<sup>3</sup>)</th>       <td>NUM(125.51ms)</td>        <td>NUM(  6.80s)</td>       <td>NUM(  107.87s)</td> <td></td></tr>
	<tr><th>gilbert </th> <th>NUM(100<sup>3</sup>)</th>       <td>NUM(121.04ms)</td>        <td>NUM(  6.71s)</td>       <td>NUM(  112.36s)</td> <td></td></tr>
	<tr class="hspc"></tr>
	<tr><th>hilbert </th> <th>NUM( 64<sup>3</sup>)</th>       <td>NUM( 94.65ms)</td>        <td>NUM(  6.04s)</td>       <td>NUM(  101.08s)</td> <td></td></tr>
	<tr><th>hilbert </th> <th>NUM(128<sup>3</sup>)</th>       <td>NUM(157.09ms)</td>        <td>NUM( 10.52s)</td>       <td>NUM(  123.02s)</td> <td></td></tr>
	<tr class="spc"></tr>

	<tr><th colspan=2>(colours)                   </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th> <th>NUM(1'103'347)</th> <th></th></tr>
	<tr><th>straight</th> <th>NUM( 64<sup>3</sup>)</th>          <td>NUM(1.21s)</td>         <td>NUM(129.69s)</td> <td>—</td>              <td>(driver timeout)</td></tr>
	<tr><th>straight</th> <th>NUM( 80<sup>3</sup>)</th>          <td>NUM(1.27s)</td>         <td>NUM(138.32s)</td> <td>NUM(   66.48s)</td> <td>(came within 200ms of a driver timeout)</td></tr>
	<tr><th>straight</th> <th>NUM( 90<sup>3</sup>)</th>          <td>NUM(1.30s)</td>         <td>NUM(143.71s)</td> <td>NUM(   52.33s)</td> <td></td></tr>
	<tr><th>straight</th> <th>NUM(100<sup>3</sup>)</th>          <td>NUM(1.78s)</td>         <td>NUM(142.43s)</td> <td>NUM(   49.49s)</td> <td></td></tr>
	<tr><th>straight</th> <th>NUM(128<sup>3</sup>)</th>          <td>NUM(2.11s)</td>         <td>NUM(190.65s)</td> <td>NUM(   67.33s)</td> <td></td></tr>
	<tr class="hspc"></tr>
	<tr><th>gilbert </th> <th>NUM( 80<sup>3</sup>)</th>          <td>NUM(1.27s)</td>         <td>NUM(148.03s)</td> <td>—</td>              <td>(driver timeout)</td></tr>
	<tr><th>gilbert </th> <th>NUM( 90<sup>3</sup>)</th>          <td>NUM(1.26s)</td>         <td>NUM(139.38s)</td> <td>NUM(   56.16s)</td> <td></td></tr>
	<tr><th>gilbert </th> <th>NUM(100<sup>3</sup>)</th>          <td>NUM(1.75s)</td>         <td>NUM(131.85s)</td> <td>NUM(   48.74s)</td> <td></td></tr>
	<tr class="hspc"></tr>
	<tr><th>hilbert </th> <th>NUM( 64<sup>3</sup>)</th>          <td>NUM(1.23s)</td>         <td>NUM(130.17s)</td> <td>—</td>              <td>(driver timeout)</td></tr>
	<tr><th>hilbert </th> <th>NUM(128<sup>3</sup>)</th>          <td>NUM(2.16s)</td>         <td>NUM(189.07s)</td> <td>NUM(   67.86s)</td> <td>(driver timeout the first time)</td></tr>
...
-->
<p class="indented continued">
MATHJAX(G=100) gilbert seems, like, kinda the best. Maybe. A little bit? This sucks.
Just <em>generating</em> all the values of <samp>decodeh</samp>
<!-- (MATHJAX(l : [0, G^3 RPAREN \cap \mathbb[Z] \to ([0, G RPAREN \cap \mathbb{Z})^3) takes NUM(3341.45ms ± 208ms)) -->
(MATHJAX(l : OVERBAR(G^3) \to \bar{G}^3) takes NUM(3341.45ms ± 208ms))
...
<div class="codecontinued">
#include "blogn_t/018.54-gilbert-gilbert-build.rs.html"
</div>

<p class="indented continued">
Why not generate <samp>[(1, 2, 3), (4, 5, 6), …]</samp> and <samp>include!("…/grid_gilbert.rs")</samp>?
Because that <samp>include!</samp> would be of a million-element array which takes NUM(~40s) (and <a href="#network-effect-rust">implies</a> like three compiler bugs).
Including MATHJAX(3\text{MB}) directly is functionally instant:
</p>
<div class="codecontinued">
#include "blogn_t/018.55-gilbert-gilbert-driver.rs.html"
</div>
<div class="singles">
	FIG(018.35-gilbert-iterorder+gilbert.png, "Same point cloud ε and section of 20x20x20 grid, but the highlighted lines wind and turn at every possible point",
	    Effective gilbert look-up order (starts near the top-left corner) for image <samp EXAMPLE_Ε_STYLE>ε</samp> with MATHJAX(G=20, r=0.05) around MATHJAX((13, 7, 16));
	    note the decreased amount of diagonal lines, corresponding to non-linearities)
</div>
...
-->
<table class="perf">
	<tr><th>(colours)                                    </th><th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->328'666)</th> <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th><q>copying</q> in axis order                 </th>     <td>NUM(  2.799ms)</td>        <td>NUM(  5.744ms)</td> <td>NUM(  8.125ms)</td> <td>NUM(279µs)/NUM(904µs)/NUM(898µs)</td></tr>
	<tr><th><q>copying</q> with <samp>GRID_GILBERT</samp></th>     <td>NUM( 10.933ms)</td>        <td>NUM( 15.397ms)</td> <td>NUM( 17.391ms)</td> <td>NUM(1.58ms)/NUM(2.94ms)/NUM(1.48ms)</td></tr>
</table>
<p class="continuing">
so, maybe,
...
<div class="codecontinued">
#include "blogn_t/018.56-gilbert-both-driver.rs.html"
</div>
...
<table class="perf left">
	...
	<tr><th>(colours)              </th><th>*NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>      <td>NUM(104.245ms)</td>          <td>NUM(6.001s)</td>        <td>NUM(106.233s)</td> <td>NUM(2.03ms)/NUM(12.84ms)/NUM(1.277s)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM(  2.88%)</td>         <td>NUM(  1.17%)</td>         <td>NUM(  3.44%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  19.34%)</td>         <td>NUM(  9.16%)</td>                    <td>—</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM(86.388ms)</td>          <td>NUM(6.579s)</td>         <td>NUM(53.748s)</td> <td>NUM(1.82ms)/NUM(14.82ms)/NUM(916.9ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM(  36.81%)</td>        <td>NUM(   1.35%)</td>         <td>NUM(  -1.05%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  28.67%)</td>        <td>NUM( -51.02%)</td>                    <td>—</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th><th>*NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>         <td>NUM(1.566s)</td>        <td>NUM(117.119s)</td>         <td>NUM(49.116s)</td> <td>NUM(11.14ms)/NUM(829.3ms)/NUM(1.62s)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM( 16.79%)</td>         <td>NUM(  4.76%)</td>         <td>NUM(  0.45%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  -7.57%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>      <td>NUM(574.617ms)</td>         <td>NUM(75.128s)</td>         <td>NUM(25.529s)</td> <td>NUM(106.6ms)/NUM(1.748ms)/NUM(982ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM(  -2.95%)</td>        <td>NUM(   3.32%)</td>        <td>NUM(  -5.71%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM( -53.88%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	SLOWDOWN_DIDNT(SLOWDOWN_DOESNT)
</table>

<div class="one-by-each dark-invert">
	FIG(018.36-gilbert-index.png,  "Plot with unlabelled x axis [0, 360] and y axis labelled index [0, 6200]; 'byaxis' looks piecewise-linear, starts around 4500 and climbs with a steady incline between x=0/x=66/x=150/x=220/x=270 but there are large jumps around those discontinuities; bygilbert stays much flatter between x=0/x=230, then jumps less, and this repeats at x=270 and x=340; adjacent_difference(byaxis) and adjacent_difference(bygilbert) are all flattened around y=0 but highlight the large discontinuities", )
	FIG(018.37-gilbert-Δindex.png, "That same plot, with y axis truncated and labelled Δindex [0, 40]; byaxis and bygilbert are unseen; adjacent_difference(byaxis) is 1 with steady bumps up to y=13 every like 10, with larger jumps around the bigger discontinuities; adjacent_difference(bygilbert) stays around y=1 throughout, with smaller fluctuations to around y=4 sometimes and similar off-screen jumps around the big discontinuities", )
</div>
...
HEADING(2, ellipsoid-vs-prism, Totally ellipsoidal, man!)
...
<div class="singles">
	<figure>
		...
		<figcaption>The prismatic bounds between MATHJAX(g_{tm} \sim g_{tM}) include NUM(~2.2×) more cells than just the cells that intersect the ellipsoid MATHJAX(R),
		            even when it's<!--'--> like half-way out-of-bounds (MATHJAX(r=0.1, G=20)).</figcaption>
	</figure>
</div>
...
MATHJAX(\frac{V_p}{V_e}) where heretofore MATHJAX(V_p = 2R_x2R_y2R_z)
but now MATHJAX(V_e = \frac{4}{3}\pi R_xR_yR_z), so
MATHJAX(\frac{8}{\frac{4}{3}\pi} \approx 47.64%).
...
<p class="indented continuing">
Thus, we seek, for a given point MATHJAX(q_t) and radius MATHJAX(r),
cubic lattice cells that intersect MATHJAX(r) around MATHJAX(q_t) or are full contained therewithin,
which, it seemed natural to me, is equivalent, for cell MATHJAX(C) to finding
closest-to-MATHJAX(q_t)    point MATHJAX(C_c \in C)
furthest-from-MATHJAX(q_t) point MATHJAX(C_f \in C)
such that at least one is inside the radius MATHJAX(|\overrightarrow{C_cq_t}| \leq r \vee |\overrightarrow{C_fq_t}| \leq r).
</p>
<p class="indented continuing">
<!-- Actually, they're<!--'-> in different coordinate spaces (MATHJAX(q_t \in P\text{ & }C \in [0, G RPAREN \cap \mathbb{Z})) -->
Actually, they're<!--'--> in different coordinate spaces (MATHJAX(q_t \in P\text{ & }C \in \bar{G}))
but we have an (almost) bijexion between them so actually
MATHJAX(|\overrightarrow{g^{-1}(C_{c \vee f})q_t}| \leq r).
MATHJAX(C)s laying on a cube lattice makes this computationally quite simple in… most cases,
since in those most cases the MATHJAX(C_c) and MATHJAX(C_f) are opposite corners, so for MATHJAX(C=(x,y,z)),
MATHJAX(C_c=(x+[    \kappa],y+[    \lambda],z+[    \mu])) and
MATHJAX(C_c=(x+[\neg\kappa],y+[\neg\lambda],z+[\neg\mu])),
with MATHJAX((\kappa, \lambda, \mu) = (C_x &lt; g(q_t)_x, \cdots))
and this seems natural and obvious.
Doing the comparison in MATHJAX(P) space frees us from the burden of figuring out how you'd<!--'--> do it on an ellipsoid post-uneven-MATHJAX(B_r) scaling.
</p>
<p class="indented continuing">
Please behold a hateful interactive javascript model of what this would look like in two dimensions.
The red point is MATHJAX(q_t), point — MATHJAX(C_c), blue — MATHJAX(C_f);
dark grey cells have neither MATHJAX(C_{c \vee f}) inside, yellow ones — one, green — both;
MATHJAX(B_r = (1,1) \Rightarrow g(x) = g^{-1}(x) = x):
</p>

#define MODELFRAME(which) <iframe src=STR(/content/assets/blogn_t/which) width="100%">No iframe. Try <a href=STR(/content/assets/blogn_t/which)>opening which directly.</a></iframe>
MODELFRAME(018.39-model-bad.html)
<script>
...
or when MATHJAX(r) doesn't<!--'--> include a corner,
or when it includes the wrong corner, or
</p>
<div class="three-by-each continuation">
	FIG(018.40-ellipsoid-vs-prism-badmodel-big.png,   "",
	    NUM((10,12)) and NUM((10,13)) should both be yellow)
	FIG(018.41-ellipsoid-vs-prism-badmodel-mid.png, "",
	    NUM((8,9)) should be yellow)
	FIG(018.42-ellipsoid-vs-prism-badmodel-small.png, "",
	    NUM((9,8)), NUM((9,9)), and NUM((10,8)) should all be yellow)
</div>
...
<div class="codecontinued">
#include "blogn_t/018.58-ellipsoid-vs-prism-badmodel-bad.hlsl.html"
</div>
<p class="continuing">
(with MATHJAX(g^{-1}) as <samp>gridpos_to_lab</samp>)
</p>
<div class="codecontinued">
#include "blogn_t/018.57-ellipsoid-vs-prism-badmodel-shader.h.hlsl.html"
</div>
...
<div class="codecontinued">
#include "blogn_t/018.59-ellipsoid-vs-prism-badmodel-good.hlsl.html"
</div>
<p class="continuation">
where we observe that MATHJAX(C_c) uses MATHJAX(g(q_t))'s components, instead of the corners', on matching axes;
MATHJAX(C_f) is additionally discriminated, on matching axes, to always pick the corner <em>away</em> from MATHJAX(q_t).
</p>

MODELFRAME(018.43-model-good.html)

<div class="three-by-each">
	FIG(018.44-ellipsoid-vs-prism-goodmodel-left.png, "",
	    MATHJAX(C = (8,8)) MATHJAX(C_y = TRUNC(g(q_t)_y) \Rightarrow C_{cy} = g(q_t)_y))
	FIG(018.45-ellipsoid-vs-prism-goodmodel-up.png,   "",
	    MATHJAX(C = (9,7)) MATHJAX(C_x = TRUNC(g(q_t)_x) \Rightarrow C_{cx} = g(q_t)_x))
	FIG(018.46-ellipsoid-vs-prism-goodmodel-mid.png,  "",
	    MATHJAX(C = (9,8)) MATHJAX(C = TRUNC(g(q_t)) \Rightarrow C_c = g(q_t)))

	FIG(018.47-ellipsoid-vs-prism-goodmodel-left-means-right.png, "",
	    MATHJAX(C = (9, 7)\text{ & }g(q_t) = (9.3, 8.5)) MATHJAX(C_x + \frac{1}{2} > g(q_t)_x))
	FIG(018.48-ellipsoid-vs-prism-goodmodel-right-means-left.png, "",
	    MATHJAX(C = (9, 7)\text{ & }g(q_t) = (9.7, 8.5)) MATHJAX(C_x + \frac{1}{2} &lt; g(q_t)_x))
	FIG(018.49-ellipsoid-vs-prism-goodmodel-right-means-centers.png, "",
	    Both axes for MATHJAX(C = TRUNC(g(q_t)) = (0, 0)))

	FIG(018.50-ellipsoid-vs-prism-goodmodel-corner.png, "",
	    NUM((10,12)) and NUM((10,13)) are both yellow)
	FIG(018.51-ellipsoid-vs-prism-goodmodel-center.png, "",
	    NUM((8,9)) is yellow)
	FIG(018.52-ellipsoid-vs-prism-goodmodel-corner.png, "",
	    NUM((9,8)), NUM((9,9)), and NUM((10,8)) are all yellow)
</div>
...
<table class="perf">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM( 95.878ms)</td>               <td>NUM( 78.837ms)</td>               <td>NUM( 82.23%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(   10.67%)</td>               <td>NUM(   42.34%)</td>               <td>NUM(-45.15 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(   25.59%)</td>               <td>NUM(   33.67%)</td>               <td>NUM(-10.02 p.p.)</td></tr>
	<tr></tr>
	<tr><th>(colours)              </th> <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM( 97.659ms)</td>               <td>NUM(125.279ms)</td>               <td>NUM(128.28%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(    7.20%)</td>               <td>NUM(    6.79%)</td>               <td>NUM( -0.55 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(    4.35%)</td>               <td>NUM(   35.69%)</td>               <td>NUM( 12.33 p.p.)</td></tr>
</table>
...
<table class="perf continued">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>      <td>NUM( 95.878ms)</td>        <td>NUM(  5.085s)</td>        <td>NUM( 44.193s)</td> <td>NUM(2ms)/NUM(13ms)/NUM(646ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM( 10.67%)</td>         <td>NUM( 15.26%)</td>         <td>NUM( 58.40%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>        <td>NUM( 25.59%)</td>         <td>NUM( 23.02%)</td>                    <td>—</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM(78.837ms)</td>         <td>NUM( 5.904s)</td>         <td>NUM(39.274s)</td> <td>NUM(656µs)/NUM(30ms)/NUM(2.26ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM(  42.34%)</td>        <td>NUM(  10.26%)</td>         <td>NUM(  26.93%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  33.67%)</td>        <td>NUM( -35.52%)</td>                    <td>—</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>       <td>NUM(  1.487s)</td>        <td>NUM( 61.931s)</td>        <td>NUM( 27.602s)</td> <td>NUM(426ms)/NUM(1.31s)/NUM(665ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM( 21.01%)</td>         <td>NUM( 47.12%)</td>         <td>NUM( 43.80%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>        <td>NUM( -2.12%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>      <td>NUM(406.214ms)</td>        <td>NUM( 52.312s)</td>        <td>NUM( 16.861s)</td> <td>NUM(19ms)/NUM(767ms)/NUM(13ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM(  27.22%)</td>        <td>NUM(  30.37%)</td>        <td>NUM(  33.96%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  -8.78%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	SLOWDOWN_DIDNT()
</table>
<p class="continuation">
so, around the predicted NUM(47%) peak. Cool.
</p>


HEADING(2, sum_cells,      commutativity can be your angle....)

<p class="indented continued">
As a side-effect we've<!--'--> classified cells into
"<!--"-->ones where no   points are within MATHJAX(r)<!--"-->",
"<!--"-->ones where some points are within MATHJAX(r)<!--"-->", and
"<!--"-->ones where all  points are within MATHJAX(r)<!--"-->".
...
<p class="continuing">
but for "<!--"-->cells where all points are within MATHJAX(r)<!--"-->"
⇔ MATHJAX((\bigwedge_{i \in \texttt{corners_inrange}}i))
⇔ MATHJAX(((x \mapsto [|\overrightarrow{xq_t}| \leq r]) = (x \mapsto 1)))
a MATHJAX(\sum) is always the same, and, thus, can be pre-computed.
In the same way that skipping iterating the outside bit led to a significant speed-up,
this can allow us to skip iterating the <em>inside</em> green bit based on just knowing MATHJAX(x_g) (<samp>gxyz</samp>) if we pre-compute the sum, so.
</p>
<div class="mathnospace">
BIG_MATHJAX_START
\begin{align}
...
(and an analogous equivalency can be drawn for MULFREQ):
</p>
<!--
g_{tf} &= g(q_t)
g_t    &= TRUNC(g(q_t)) \Leftrightarrow g_t = TRUNC(g_{tf})
...
-->
<div class="codecontinued">
#include "blogn_t/018.60-sum_cells-shader.h.hlsl.html"
</div>
<p class="continuing">
as computed by
...
<div class="codecontinued">
#include "blogn_t/018.61-sum_cells.kernel.hlsl.inc.hlsl.html"
</div>
<p class="continuing">
and
...
<div class="codecontinued">
#include "blogn_t/018.62-sum_cells.straight.hlsl.html"
</div>
<p class="continuing">
or
...
<div class="codecontinued">
#include "blogn_t/018.63-sum_cells.oneshot.hlsl.html"
</div>
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.64-sum_cells-driver.rs.html"
</div>
<p class="continuing">
...
MATHJAX(G^3 SIZEOF((uint, uint)) + \varepsilon \approx 8\text{MB} \approx 3\%)
to
MATHJAX(G^3 (SIZEOF((uint, uint)) + 2 SIZEOF((float3, uint))) + \varepsilon \approx 40\text{MB} \approx 15\%)
so it better be worth it!
(Even though the additional MATHJAX(32\text{MB}) doesn't<!--'--> need to be in <code>DEVICE_LOCAL & HOST_VISIBLE</code> at all.)
...
<table class="perf">
	<tr><th>(colours)                           </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>memcpy                              </th>        <td>NUM(220.5µs)</td>         <td>NUM(268.7µs)</td>         <td>NUM(345.5µs)</td> <td>NUM(57µs)/NUM(64µs)/NUM(62µs)</td></tr>
	<tr><th>memcpy + <samp>sum_cells.hlsl</samp></th>        <td>NUM(244.6µs)</td>         <td>NUM(338.8µs)</td>         <td>NUM(507.3µs)</td> <td>NUM(56µs)/NUM(58µs)/NUM(37µs)</td></tr>
	<tr><th>(cost)                              </th>          <td>NUM( 24µs)</td>           <td>NUM( 70µs)</td>           <td>NUM(162µs)</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)                           </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>memcpy                              </th>        <td>NUM(248.4µs)</td>         <td>NUM(409.5µs)</td>         <td>NUM(871.7µs)</td> <td>NUM(27µs)/NUM(71µs)/NUM(69µs)</td></tr>
	<tr><th>memcpy + <samp>sum_cells.hlsl</samp></th>        <td>NUM(270.9µs)</td>         <td>NUM(641.1µs)</td>       <td>NUM(  1.597ms)</td> <td>NUM(42µs)/NUM(34µs)/NUM(30µs)</td></tr>
	<tr><th>(cost)                              </th>          <td>NUM( 22µs)</td>           <td>NUM(232µs)</td>           <td>NUM(725µs)</td> <td></td></tr>
</table>
...
<div class="codecontinued">
#include "blogn_t/018.65-sum_cells-kernel.hlsl.html"
</div>
<p class="continuing">
where, expectedly,
<a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/180a7268c7d82d5b4ed1bc4d53488288d49f8c39/item/src/posterise_grid.hlsl"><samp>posterise_grid.hlsl</samp></a>/\
<a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/180a7268c7d82d5b4ed1bc4d53488288d49f8c39/item/src/posterise_grid_oneshot.hlsl"><samp>posterise_grid_oneshot.hlsl</samp></a>:
</p>
<div class="codecontinued">
<pre class="language-hlsl"><code class="language-hlsl">\
<span class="token macro property"><span class="token directive keyword">#define</span> <span class="token macro-name">SUMS_FIELD</span> <span class="token expression">group_sums</span></span>
</code></pre>
</div>
<p class="continuing">
and
<a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/180a7268c7d82d5b4ed1bc4d53488288d49f8c39/item/src/posterise_grid.mul.hlsl"><samp>posterise_grid.mul.hlsl</samp></a>/\
<a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/180a7268c7d82d5b4ed1bc4d53488288d49f8c39/item/src/posterise_grid_oneshot.mul.hlsl"><samp>posterise_grid_oneshot.mul.hlsl</samp></a>:
</p>
<div class="codecontinued">
<pre class="language-hlsl"><code class="language-hlsl">\
<span class="token macro property"><span class="token directive keyword">#define</span> <span class="token macro-name">SUMS_FIELD</span> <span class="token expression">group_mulsums</span></span>
</code></pre>
</div>
<p></p>
...
<table class="perf">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th> <th>NUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM( 68.781ms)</td>               <td>NUM( 53.958ms)</td>               <td>NUM( 78.45%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(   39.40%)</td>               <td>NUM(   46.11%)</td>               <td>NUM( -3.78 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(   46.62%)</td>               <td>NUM(   54.60%)</td>               <td>NUM(-13.80 p.p.)</td></tr>
	<tr></tr>
	<tr><th>(colours)              </th> <th>DUM(<!--'-->16'755), PALETTE</th> <th>DUM(<!--'-->16'755), MULFREQ</th> <th>(MULFREQ/PALETTE)</th></tr>
	<tr><th>5700 XT                </th> <td>NUM( 68.856ms)</td>               <td>NUM( 83.216ms)</td>               <td>NUM(120.85%)</td></tr>
	<tr><th>(speed-up vs previous) </th> <td>NUM(   29.49%)</td>               <td>NUM(   33.58%)</td>               <td>NUM( -7.42 p.p.)</td></tr>
	<tr><th>(speed-up vs base-line)</th> <td>NUM(   47.43%)</td>               <td>NUM(   45.20%)</td>               <td>NUM( -4.90 p.p.)</td></tr>
</table>
...
<table class="perf continued">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>      <td>NUM( 68.781ms)</td>        <td>NUM(  3.242s)</td>        <td>NUM( 29.624s)</td> <td>NUM(968µs)/NUM(6.8ms)/NUM(190ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM( 28.26%)</td>         <td>NUM( 46.82%)</td>         <td>NUM( 32.97%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>        <td>NUM( 50.93%)</td>         <td>NUM( 23.02%)</td>                    <td>—</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>       <td>NUM(53.958ms)</td>         <td>	NUM( 3.475s)</td>        <td>NUM( 19.245s)</td> <td>NUM(312µs)/NUM(15ms)/NUM(440ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM(  31.56%)</td>        <td>NUM(  41.15%)</td>        <td>NUM(  51.00%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  55.45%)</td>        <td>NUM(  20.25%)</td>                    <td>—</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>       <td>NUM(  1.154s)</td>        <td>NUM( 38.160s)</td>        <td>NUM( 22.638s)</td> <td>NUM(253ms)/NUM(394ms)/NUM(1198ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM( 22.34%)</td>         <td>NUM( 38.38%)</td>         <td>NUM( 17.98%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>        <td>NUM( 20.70%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>      <td>NUM(299.399ms)</td>        <td>NUM( 30.320s)</td>        <td>NUM( 12.264s)</td> <td>NUM(2.9ms)/NUM(381ms)/NUM(826ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM(  26.30%)</td>        <td>NUM(  42.04%)</td>        <td>NUM(  27.26%)</td> <td></td></tr>
	<tr><th>(speed-up vs base-line)</th>       <td>NUM(  19.82%)</td>                    <td>—</td>                    <td>—</td> <td></td></tr>
	SLOWDOWN_DIDNT()
</table>

<div class="singles">
FIG(018.53-graphml-sum_cells.png, "", )
</div>


HEADING(3, sum_cells+ulp, or yuor devil)

<p class="continued">
The individual values of
MATHJAX(\sum      h(x_g) )
go quite a ways away from NUM(0):
</p>
<table class="Ppop">
	<caption>MATHJAX(G=100)</caption>
	<tr><th></th>                  <th>most populous cell, PALETTE (MATHJAX(\sigma_{lab}), </th>          <td>MATHJAX(\sigma_{freq}))</td> <th>most populous cell, MULFREQ (MATHJAX(\sigma_{lab}), </th>            <td>MATHJAX(\sigma_{freq}))</td></tr>
	<tr><td EXAMPLE_Α_STYLE>α</td> <td>(NUM((44.041176, -0.31927294, -0.34573233)), </td>                                <td>NUM(45))</td> <td>(NUM((9383.919,   -65.55733,     -20.474636)),    </td>                        <td>NUM(   9434))</td></tr>
	<tr><td EXAMPLE_Β_STYLE>β</td> <td>(NUM((58.839714, -0.22819409, -0.020030022)),</td>                                <td>NUM(60))</td> <td>(NUM((10189.065,   -0.47646168,   -0.102591455)), </td>                        <td>NUM(  10203))</td></tr>
	<tr><td EXAMPLE_Γ_STYLE>γ</td> <td>(NUM((18.17284,   0.18759891,  0.18187594)), </td>                                <td>NUM(21))</td> <td>(NUM((39128.875,   -0.0036974102, -0.0036974102)),</td>                        <td>NUM( 248129))</td></tr>
	<tr><td EXAMPLE_Δ_STYLE>δ</td> <td>(NUM((3.2355795,  1.158621,    0.64843106)), </td>                                <td>NUM( 6))</td> <td>(NUM((0,            0,             0)),           </td>                        <td>NUM(1716920))</td></tr>
	<tr><td EXAMPLE_Ε_STYLE>ε</td> <td>(NUM((55.92771,  -0.81256634, -0.09105891)), </td>                                <td>NUM(59))</td> <td>(NUM((6374.0493, 2605.977,      -906.254)),       </td>                        <td>NUM(   9462))</td></tr>
	<tr style="height: 0.5em;"></tr>
	<tr><th></th>                  <th>cell with largest MATHJAX(l), PALETTE (MATHJAX(\sigma_{lab}), </th> <td>MATHJAX(\sigma_{freq}))</td> <th>cell with largest MATHJAX(l), MULFREQ (MATHJAX(\sigma_{lab}), </th> <td>MATHJAX(\sigma_{freq}))</td></tr>
	<tr><td EXAMPLE_Α_STYLE>α</td> <td>(NUM((44.041176, -0.31927294, -0.34573233)), </td>                                 <td>NUM(45))</td> <td>(NUM((9383.919,   -65.55733,    -20.474636)),    </td>                        <td>NUM(   9434))</td></tr>
	<tr><td EXAMPLE_Β_STYLE>β</td> <td>(NUM((58.839714, -0.22819409, -0.020030022)),</td>                                 <td>NUM(60))</td> <td>(NUM((10189.065,   -0.47646168,  -0.102591455)), </td>                        <td>NUM(  10203))</td></tr>
	<tr><td EXAMPLE_Γ_STYLE>γ</td> <td>(NUM((18.461334,  0.1365368,   0.25196123)), </td>                                 <td>NUM(20))</td> <td>(NUM((52687.547,   -0.0033568442, 0.0033609867)),</td>                        <td>NUM( 112776))</td></tr>
	<tr><td EXAMPLE_Δ_STYLE>δ</td> <td>(NUM((3.2355795,  1.158621,    0.64843106)), </td>                                 <td>NUM( 6))</td> <td>(NUM((20940.623, 7498.585,     4196.6367)),      </td>                        <td>NUM(  70065))</td></tr>
	<tr><td EXAMPLE_Ε_STYLE>ε</td> <td>(NUM((55.92771,  -0.81256634, -0.09105891)), </td>                                 <td>NUM(59))</td> <td>(NUM((6374.0493, 2605.977,     -906.254)),       </td>                        <td>NUM(   9462))</td></tr>
	<tr style="height: 0.5em;"></tr>
	<tr><th></th>                  <th>cell with largest MATHJAX(a), PALETTE (MATHJAX(\sigma_{lab}), </th> <td>MATHJAX(\sigma_{freq}))</td> <th>cell with largest MATHJAX(a), MULFREQ (MATHJAX(\sigma_{lab}), </th> <td>MATHJAX(\sigma_{freq}))</td></tr>
	<tr><td EXAMPLE_Α_STYLE>α</td> <td>(NUM((8.910748,  -1.357208,  -0.7054685)), </td>                                 <td>NUM(13))</td> <td>(NUM((6691.933,  -82.91916,  -29.420803)), </td>                                <td>NUM(   6763))</td></tr>
	<tr><td EXAMPLE_Β_STYLE>β</td> <td>(NUM((2.79438,   -0.4932925,  0.46539965)),</td>                                 <td>NUM( 4))</td> <td>(NUM((823.4114,  -1.3063359, -0.29277617)),</td>                                <td>NUM(    826))</td></tr>
	<tr><td EXAMPLE_Γ_STYLE>γ</td> <td>(NUM((10.627919,  0.5354426,  0.28728706)),</td>                                 <td>NUM(13))</td> <td>(NUM((2371.3867, 629.1607,   -65.39355)),  </td>                                <td>NUM(  10809))</td></tr>
	<tr><td EXAMPLE_Δ_STYLE>δ</td> <td>(NUM((3.2355795,  1.158621,   0.64843106)),</td>                                 <td>NUM( 6))</td> <td>(NUM((20940.623, 7498.585,   4196.6367)),  </td>                                <td>NUM(  70065))</td></tr>
	<tr><td EXAMPLE_Ε_STYLE>ε</td> <td>(NUM((3.4370112,  1.2792559, -0.44643825)),</td>                                 <td>NUM( 5))</td> <td>(NUM((6374.0493, 2605.977,   -906.254)),   </td>                                <td>NUM(   9462))</td></tr>
	<tr style="height: 0.5em;"></tr>
	<tr><th></th>                  <th>cell with largest MATHJAX(b), PALETTE (MATHJAX(\sigma_{lab}), </th> <td>MATHJAX(\sigma_{freq}))</td> <th>cell with largest MATHJAX(b), MULFREQ (MATHJAX(\sigma_{lab}), </th> <td>MATHJAX(\sigma_{freq}))</td></tr>
	<tr><td EXAMPLE_Α_STYLE>α</td> <td>(NUM((31.815504, -0.57158244, -0.95032257)), </td>                                 <td>NUM(34))</td> <td>(NUM((680.8264,   -66.88674,   -58.81892)), </td>                        <td>NUM(    982))</td></tr>
	<tr><td EXAMPLE_Β_STYLE>β</td> <td>(NUM((3.7039335, -0.46457234, 0.48276806)),  </td>                                 <td>NUM( 5))</td> <td>(NUM((107.945366, -0.42522368, 0.8002589)), </td>                        <td>NUM(    117))</td></tr>
	<tr><td EXAMPLE_Γ_STYLE>γ</td> <td>(NUM((8.998497,  0.1350762,   0.34426308)),  </td>                                 <td>NUM(11))</td> <td>(NUM((21374.111,  178.88039,   555.86554)), </td>                        <td>NUM(  24665))</td></tr>
	<tr><td EXAMPLE_Δ_STYLE>δ</td> <td>(NUM((2.0014775, -0.1437166,  -1.3794196)),  </td>                                 <td>NUM( 6))</td> <td>(NUM((7937.199,   -569.93335,  -5470.323)), </td>                        <td>NUM(  42118))</td></tr>
	<tr><td EXAMPLE_Ε_STYLE>ε</td> <td>(NUM((15.066136, 0.088873774, 2.2854338)),   </td>                                 <td>NUM(17))</td> <td>(NUM((6374.0493,  2605.977,    -906.254)),  </td>                        <td>NUM(   9462))</td></tr>
</table>
<p class="continuing">
and especially, since MATHJAX(P) is symmetric around NUM(0) in the other axes, in the MATHJAX(l) axis.
This wouldn't be an issue, except, <a href="//tom7.org/grad/">famously</a>, floats aren't actually commutative,
and they're<!--'--> less commutative the further away from NUM(0) they are.
</p>
<p class="indented continuing">
...
<div class="codecontinued">
#include "blogn_t/018.66-sum_cells+ulp-cond.hlsl.html"
</div>
<p class="continuing">
never trigger, and, thus, MATHJAX(O) never converges.
...
<div class="codecontinued">
#include "blogn_t/018.67-sum_cells+ulp-instrument.hlsl.html"
</div>
<p class="continuing">
yields
</p>
...
<p class="continuing">
where we observe a NUM(2)-long cycle, where <code>center.l == nextafter(nextafter(newcenter.l))</code> (2 ULPs off) and v.v..
This can be mitigated by upgrading the MATHJAX(f) condition:
</p>
<div class="mathnospace real">
BIG_MATHJAX_START
\begin{align}
...
<div class="codecontinued">
#include "blogn_t/018.68-sum_cells+ulp-fix.hlsl.html"
</div>
...
HEADING_S(2, occupancy, class="continuing", re-tuning the piece-meal iteration count)
<p class="continuing">
since more than <a href="#oneshot">previously</a> can be done without exceeding NUM(8s);
a doubling of
MATHJAX(l \mapsto \begin{cases}
10     &\text{if}\ l \in [100'000,  \infty<!--'--> RPAREN\\<!---->
20     &\text{if}\ l \in [ 10'000, 100'000         RPAREN\\<!---->
\infty &\text{if}\ l \in [      0,  10'000<!--'--> RPAREN\\<!---->
\end{cases})
...
<table class="perf continuation">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>       <td>NUM(  3.048s)</td>        <td>NUM( 31.908s)</td> <td>NUM(6.3ms)/NUM(591ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM(  5.98%)</td>         <td>NUM( -7.71%)</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>        <td>NUM( 3.264s)</td>         <td>NUM(21.208s)</td> <td>NUM(12.9ms)/NUM(959ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>       <td>NUM(   6.06%)</td>        <td>NUM( -10.20%)</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)              </th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, PALETTE       </th>        <td>NUM( 39.261s)</td>        <td>NUM( 20.687s)</td> <td>NUM(413ms)/NUM(1300ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>         <td>NUM( -2.89%)</td>         <td>NUM(  8.62%)</td> <td></td></tr>
	<tr><th>5700 XT, MULFREQ       </th>        <td>NUM( 29.622s)</td>        <td>NUM( 11.472s)</td> <td>NUM(483ms)/NUM(471ms)</td></tr>
	<tr><th>(speed-up vs previous) </th>        <td>NUM(   2.30%)</td>        <td>NUM(   6.46%)</td> <td></td></tr>
</table>
...
HEADING_S(2, grid-size-again, class="continued", Grid size re-evaluation)
<p class="continuing">
...
<table class="perf continuation">
	<tr><th>(colours)                </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, MATHJAX(G =  64)</th>      <td>NUM(  58.852ms)</td>                   <td>!</td>        <td>NUM( 37.512s)</td> <td>NUM(2.49ms)/NUM(875ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G =  80)</th>      <td>NUM(  64.827ms)</td>       <td>NUM(  2.983s)</td>        <td>NUM( 34.030s)</td> <td>NUM(1.96ms)/NUM(9.01ms)/NUM(903ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G =  90)</th>      <td>NUM(  66.530ms)</td>       <td>NUM(  3.000s)</td>        <td>NUM( 32.443s)</td> <td>NUM(1.97ms)/NUM(17.5ms)/NUM(96.4ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G = 100)</th>      <td>NUM(  68.781ms)</td>       <td>NUM(  3.242s)</td>        <td>NUM( 29.624s)</td> <td>NUM(968µs)/NUM(6.8ms)/NUM(190ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G = 110)</th>      <td>NUM(  81.218ms)</td>       <td>NUM(  3.122s)</td>        <td>NUM( 31.146s)</td> <td>NUM(3.94ms)/NUM(7.95ms)/NUM(854ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G = 128)</th>      <td>NUM(  95.885ms)</td>       <td>NUM(  3.870s)</td>        <td>NUM( 35.580s)</td> <td>NUM(1.94ms)/NUM(14.39ms)/NUM(396ms)</td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)                </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, MATHJAX(G =  64)</th>       <td>NUM(750.507ms)</td>       <td>NUM( 45.496s)</td>        <td>NUM( 27.624s)</td> <td>NUM(52.99ms)/NUM(460ms)/NUM(489ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G =  80)</th>       <td>NUM(958.600ms)</td>       <td>NUM( 41.959s)</td>        <td>NUM( 23.934s)</td> <td>NUM(296ms)/NUM(759ms)/NUM(767ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G =  90)</th>       <td>NUM(869.838ms)</td>       <td>NUM( 41.072s)</td>                    <td>!</td> <td>NUM(5.09ms)/NUM(414ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G = 100)</th>       <td>NUM(   1.154s)</td>       <td>NUM( 38.160s)</td>        <td>NUM( 22.638s)</td> <td>NUM(253ms)/NUM(394ms)/NUM(1198ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G = 110)</th>       <td>NUM(   1.500s)</td>       <td>NUM( 38.148s)</td>        <td>NUM( 19.301s)</td> <td>NUM(487ms)/NUM(748ms)/NUM(735ms)</td></tr>
	<tr><th>5700 XT, MATHJAX(G = 128)</th>       <td>NUM(   1.758s)</td>       <td>NUM( 44.541s)</td>                    <td>!</td> <td>NUM(479ms)/NUM(815ms)</td></tr>
	SLOWDOWN_DIDNT_GENERIC(<strong>!</strong>: timed out)
</table>

<p>
MATHJAX(G = 100) stays winning.
</p>


HEADING_S(2, gxyz-unnest, class="continued", Failure: un-nesting iteration of points-in-cell from iteration of cells-in-bounding-box)
<!-- https://discord.com/channels/145079846832308224/150317232239542272/1345280142754189463 -->
#define RATCHETFREAK   <strong style="color: rgb(46, 204, 113);">ratchetfreak</strong>
#define NABIJACZLEWELI <strong style="color: rgb(228, 85, 182);">((наб *)(войд)())()</strong>
<blockquote class="continuing">\
<date>[07:23]</date> RATCHETFREAK: have you considered doing the rejig of the logic I mentioned above, splitting the looking for next shell cell and iterating a shell cell instead of one nested in the other?
<hr />\
<date>[20:50]</date> NABIJACZLEWELI: i don't really see how this is different<!--'-->
<date>[20:51]</date> NABIJACZLEWELI: or, rather, this seems to me like it's exactly the same loop but with two gotos except with the state spilled<!--'-->
<hr />\
<date>[21:02]</date> RATCHETFREAK: it is the same loop but it means that in a warp if one thread is ready to iterate a set of points the rest doesn't have to wait until it's done to continue to the next set
<!---->
...
<date>[21:17]</date> RATCHETFREAK: summerized the most expensive operation in your compute shader is iterating points,
<!---->
...
<div class="codecontinued">
#include "blogn_t/018.69-gxyz-unnest.first.hlsl.html"
</div>
...
<table class="perf">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(397.024ms)</td>              </tr>
	<tr><th>(speed-up vs previous) </th> <td>ONEOVER(5.772)NUM(×)</td>        </tr>
	<tr><th>(speed-up vs base-line)</th> <td>ONEOVER(3.070)NUM(×)</td>        </tr>
</table>
<blockquote class="continuing codecontinued" style="white-space: pre-wrap;">\
<date>[21:52]</date> RATCHETFREAK: ah, was worth a shot...
<date>[21:53]</date> NABIJACZLEWELI: no for sure
<date>[21:55]</date> RATCHETFREAK: seen = dot(gxyz, uint3(10000, 100, 1))
<date>[21:56]</date> NABIJACZLEWELI: hm yeah i suppose but only the inverse is of interest right
<date>[21:59]</date> NABIJACZLEWELI: keeping gxyz and bumping .z and propagating overflow had more branches in it when i wrote it
<date>[21:59]</date> NABIJACZLEWELI: but maybe im not branchlesspilled enough
<date>[22:02]</date> RATCHETFREAK: hmm
<pre class="language-hlsl"><code class="language-hlsl"><span class="token comment">[diff for below]</span></code></pre>\
<date>[22:04]</date> NABIJACZLEWELI: zamn
<date>[22:06]</date> RATCHETFREAK: only 1 branch and it's<!--'--> the branch you want to happen
<date>[22:08]</date> RATCHETFREAK: bonus it also avoid the idiv that's<!--'--> typically pretty expensive
</blockquote>
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.70-gxyz-unnest.second.hlsl.html"
</div>
<!--
...
<table class="perf left">
	<tr><th>(colours)              </th> <th>NUM(<!--'-->16'755), PALETTE</th></tr>
	<tr><th>5700 XT                </th> <td>NUM(397.024ms)</td>              </tr>
	<tr><th>(speed-up vs above)    </th> <td>   NUM(50.80%)</td>              </tr>
	<tr><th>(speed-up vs previous) </th> <td>ONEOVER(3.828)NUM(×)</td>        </tr>
	<tr><th>(speed-up vs base-line)</th> <td>ONEOVER(2.036)NUM(×)</td>        </tr>
</table>

<blockquote class="continuing codecontinued">\
<date>[22:25]</date> NABIJACZLEWELI: also funny, too me, that you've<!--'--> reimplemented wrapping addition combinatorially to solve what is otherwise a very mundane problem
<date>[22:26]</date> RATCHETFREAK: wrapping addition with carry lookahead
</blockquote>


HEADING_S(2, gxyz-xyz, class="continued", Failure: MATHJAX(l&lt;a&lt;b))
<!--
...
-->
<blockquote class="continuing">\
<date>[14:54]</date> RATCHETFREAK: for example a very flat in the z-axis data set (max_z - min_z < radius) will have a gr_z == 100 and always iterate the full depth of the grid on each particle
<hr />\
<date>[14:55]</date> NABIJACZLEWELI: this will always be the case
<date>[14:56]</date> NABIJACZLEWELI: if you need to look at the whole z-axis then you need to look at the whole z-axis
<hr />\
<date>[14:56]</date> RATCHETFREAK: but if it's<!--'--> flat in the x axis it will need to deal with 2 more inner loops
<date>[14:57]</date> RATCHETFREAK: so a rotation of the data set might be in order so the flattest axis is always z
<date>[14:57]</date> RATCHETFREAK: that way the longest inner loop is inside\
</blockquote>
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.73-gxyz-xyz-assign_cell.hlsl.html"
</div>
<p class="continuing">
and
<a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/sorted-coords/item/src/posterise_grid.kernel.hlsl.inc#L87"><samp>posterise_grid.kernel.hlsl.inc</samp></a>/\
<a href="//git.sr.ht/~nabijaczleweli/mean-shift-clusterer/tree/sorted-coords/item/src/posterise_grid_oneshot.kernel.hlsl.inc#L86"><samp>posterise_grid_oneshot.kernel.hlsl.inc</samp></a>
can unrotate them when uploading <samp>outputs</samp>:
</p>
<div class="codecontinued">
#include "blogn_t/018.74-gxyz-xyz-kernel.hlsl.html"
</div>
<p class="continuation">
since everything else is MATHJAX(P)-agnostic.
</p>

<table class="perf left">
	<tr><th>(colours)                </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, natural         </th>       <td>NUM(68.781ms)</td>         <td>NUM( 3.242s)</td>         <td>NUM(29.624s)</td> <td>NUM(968µs)/NUM(6.8ms)/NUM(190ms)</td></tr>
	<tr><th>5700 XT, NUM(l&lt;a&lt;b)</th>       <td>NUM(69.741ms)</td>         <td>NUM( 3.057s)</td>         <td>NUM(32.232s)</td> <td>NUM(2.17ms)/NUM(11.1ms)/NUM(780ms)</td></tr>
	<tr><th>5700 XT, NUM(l&gt;a&gt;b)</th>       <td>NUM(71.622ms)</td>         <td>NUM( 3.143s)</td>         <td>NUM(34.152s)</td> <td>NUM(2.06ms)/NUM(7.73ms)/NUM(2.061s)</td></tr>
	<tr></tr>
	<tr><th>(colours)                </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, natural         </th>        <td>NUM( 1.154s)</td>         <td>NUM(38.160s)</td>         <td>NUM(22.638s)</td> <td>NUM(253ms)/NUM(394ms)/NUM(1198ms)</td></tr>
	<tr><th>5700 XT, NUM(l&lt;a&lt;b)</th>        <td>NUM( 1.374s)</td>         <td>NUM(39.296s)</td>         <td>NUM(20.306s)</td> <td>NUM(487ms)/NUM(819ms)/NUM(641ms)</td></tr>
	<tr><th>5700 XT, NUM(l&gt;a&gt;b)</th>        <td>NUM( 1.401s)</td>         <td>NUM(39.200s)</td>         <td>NUM(20.292s)</td> <td>NUM(487ms)/NUM(827ms)/NUM(768ms)</td></tr>
</table>
...
HEADING_S(2, groups_accumulator-mutex, class="continuing", parallelising is points-in-cell <samp>assign_cell.hlsl</samp> <q>grouping</q> post-processing)
<p class="continuing">
...
<div class="codecontinued">
#include "blogn_t/018.75-groups_accumulator-mutex-driver.rs.html"
</div>
...
<table class="perf left">
	<tr><th>(colours)                               </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>i7-2600, <q>grouping</q>, serial        </th>       <td>NUM( 4.453ms)</td>        <td>NUM(11.115ms)</td>        <td>NUM(23.344ms)</td> <td>NUM(327µs)/NUM(686µs)/NUM(1.847ms)</td></tr>
	<tr><th>i7-2600, <q>grouping</q>, NUM(8) threads</th>       <td>NUM( 5.275ms)</td>        <td>NUM( 7.067ms)</td>        <td>NUM( 9.819ms)</td> <td>NUM(634µs)/NUM(788µs)/NUM(1.865ms)</td></tr>
	<tr><th>i7-2600, <q>copying</q>,  serial        </th>       <td>NUM( 2.763ms)</td>        <td>NUM(11.469ms)</td>        <td>NUM(12.275ms)</td> <td>NUM(209µs)/NUM(1.256ms)/NUM(825µs)</td></tr>
	<tr><th>i7-2600, <q>copying</q>,  NUM(8) threads</th>       <td>NUM( 3.461ms)</td>        <td>NUM(14.873ms)</td>        <td>NUM(15.560ms)</td> <td>NUM(478µs)/NUM(1.780ms)/NUM(1.557ms)</td></tr>
	<tr><th>(speed-up)                              </th>       <td>NUM( -21.07%)</td>        <td>NUM(   2.86%)</td>        <td>NUM(  28.75%)</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)                               </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>i7-2600, <q>grouping</q>, serial        </th>       <td>NUM( 5.699ms)</td>        <td>NUM(31.908ms)</td>        <td>NUM(94.578ms)</td> <td>NUM(699µs)/NUM(1.523ms)/NUM(8.194ms)</td></tr>
	<tr><th>i7-2600, <q>grouping</q>, NUM(8) threads</th>       <td>NUM( 5.178ms)</td>        <td>NUM(11.376ms)</td>        <td>NUM(24.370ms)</td> <td>NUM(507µs)/NUM(1.869ms)/NUM(5.247ms)</td></tr>
	<tr><th>i7-2600, <q>copying</q>,  serial        </th>       <td>NUM( 2.937ms)</td>        <td>NUM(14.389ms)</td>        <td>NUM(16.953ms)</td> <td>NUM(323µs)/NUM(1.111ms)/NUM(1.347ms)</td></tr>
	<tr><th>i7-2600, <q>copying</q>,  NUM(8) threads</th>       <td>NUM( 3.327ms)</td>        <td>NUM(17.073ms)</td>        <td>NUM(20.331ms)</td> <td>NUM(221µs)/NUM(1.382ms)/NUM(836µs)</td></tr>
	<tr><th>(speed-up)                              </th>       <td>NUM(   1.52%)</td>        <td>NUM(  38.55%)</td>        <td>NUM(  59.92%)</td> <td></td></tr>
</table>
...
<table class="perf">
	<tr><th>(colours)                                                                 </th> <th>NUM(<!--'-->16'755)</th>  <th>NUM(<!--'-->84'860)</th> <th>NUM(<!--'-->242'931)</th> <th>±σ</th></tr>
	<tr><th>i7-2600, <q>grouping</q>, NUM(8) threads, <code>.unwrap_unchecked()</code></th>       <td>NUM( 5.118ms)</td>        <td>NUM( 6.712ms)</td>        <td>NUM( 8.808ms)</td> <td>NUM(534µs)/NUM(821µs)/NUM(1.078ms)</td></tr>
	<tr><th>i7-2600, <q>copying</q>,  NUM(8) threads, <code>.unwrap_unchecked()</code></th>       <td>NUM( 3.431ms)</td>        <td>NUM(12.855ms)</td>        <td>NUM(13.936ms)</td> <td>NUM(491µs)/NUM(1.278ms)/NUM(657µs)</td></tr>
	<tr><th>(speed-up vs serial)                                                      </th>       <td>NUM( -18.47%)</td>        <td>NUM(  13.36%)</td>        <td>NUM(  36.15%)</td> <td></td></tr>
	<tr><th>(speed-up vs serial)                                                      </th>     <td>NUM( 2.60 p.p.)</td>      <td>NUM(10.50 p.p.)</td>      <td>NUM( 7.40 p.p.)</td> <td></td></tr>
	<tr><th>(speed-up vs NUM(8) threads)                                              </th>       <td>NUM(   2.15%)</td>        <td>NUM(  10.81%)</td>        <td>NUM(  10.38%)</td> <td></td></tr>
	<tr class="spc"></tr>
	<tr><th>(colours)                                                                 </th> <th>NUM(<!--'-->27'868)</th> <th>NUM(<!--'-->328'666)</th>       <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>i7-2600, <q>grouping</q>, NUM(8) threads, <code>.unwrap_unchecked()</code></th>       <td>NUM( 5.148ms)</td>        <td>NUM(11.655ms)</td>        <td>NUM(25.468ms)</td> <td>NUM(450µs)/NUM(2.112ms)/NUM(5.812ms)</td></tr>
	<tr><th>i7-2600, <q>copying</q>,  NUM(8) threads, <code>.unwrap_unchecked()</code></th>       <td>NUM( 3.353ms)</td>        <td>NUM(16.309ms)</td>        <td>NUM(20.246ms)</td> <td>NUM(368µs)/NUM(1.725ms)/NUM(1.636ms)</td></tr>
	<tr><th>(speed-up vs serial)                                                      </th>       <td>NUM(   1.56%)</td>        <td>NUM(  39.60%)</td>        <td>NUM(  59.01%)</td> <td></td></tr>
	<tr><th>(speed-up vs serial)                                                      </th>     <td>NUM( 0.04 p.p.)</td>      <td>NUM( 1.05 p.p.)</td>      <td>NUM(-0.91 p.p.)</td> <td></td></tr>
	<tr><th>(speed-up vs NUM(8) threads)                                              </th>       <td>NUM(   0.04%)</td>        <td>NUM(   1.71%)</td>        <td>NUM(  -2.26%)</td> <td></td></tr>
</table>

<p>
Not great, not terrible, for the cost of bumping <samp>groups_accumulator</samp> from is a similar size to the MATHJAX(8\text{MB}) —
MATHJAX(G^3 \cdot SIZEOF(Mutex&lt;Vec&lt;_>>) = G^3 \cdot (3 SIZEOF(usize) + m) = 32\text{MB}_{\text{LP64}} \vee 20\text{MB}_{\text{ILP32}_\text{linux}} \vee 16\text{MB}_{\text{ILP32}_\text{Win32}})
(<a href="#grid-grid">from<!-- -->MATHJAX(\ 24\text{MB}_\text{LP64} \vee 12\text{MB}_\text{ILP32})</a>).
</p>
...
HEADING_S(2, groupshared, class="continuing", de-parallelising <samp>find_bbox.hlsl</samp>)
<p class="continuing">
was also in order.
...
"<a href="//learn.microsoft.com/en-us/windows/win32/direct3dhlsl/waveallmin">reducing</a><!--"-->
 ...
 in a wave"<!--"-->
(HLSL   nomenclature).
I think it's the bit that's NUM(64) (or NUM(32) on n<span style="font-variant: small-caps;">vidia</span> cards)?
...
<div class="codecontinued">
#include "blogn_t/018.76-groupshared-find_bbox.hlsl.html"
</div>
<p class="continuing">
but this fails to compile with
</p>
<blockquote class="continuation"><pre class="continuation" style="white-space: pre-wrap;">\
...
<div class="codecontinued">
#include "blogn_t/018.77-groupshared-build.rs.html"
</div>

<p class="indented continued">
It may also be apt to validate this is supported (though it is likely also an over-abundance of caution),
which can only be queried by asking for the physical device properties… next <em>NUM(2)</em> the subgroup properties.
...
<div class="codecontinued">
#include "blogn_t/018.78-groupshared-driver.rs.html"
</div>
<p class="continuation">
...
and
<a href="//registry.khronos.org/SPIR-V/specs/unified1/SPIRV.html#OpGroupNonUniformUMin" ><code>OpGroupNonUniformUMin</code></a>/\
<a href="//registry.khronos.org/SPIR-V/specs/unified1/SPIRV.html#OpGroupNonUniformUMax" ><code>Max</code></a>,
...
<table class="perf left">
	<tr><th>(colours)       </th> <th>NUM(<!--'-->16'755)</th> <th>NUM(1'103'347)</th> <th>±σ</th></tr>
	<tr><th>5700 XT, natural</th>      <td>NUM( 74.897µs)</td> <td>NUM(156.374µs)</td> <td>NUM(20µs)/NUM(81µs)</td></tr>
	<tr><th>5700 XT, wavy   </th>      <td>NUM( 76.884µs)</td> <td>NUM(141.892µs)</td> <td>NUM(20µs)/NUM(59µs)</td></tr>
	<tr><th>(speed-up)      </th>      <td>NUM(   -2.65%)</td> <td>NUM(    9.26%)</td> <td></td></tr>
</table>
...
HEADING(2, network-effect, Network effect)
#define DIG(...)          <span style="font-family: 'RobotoSlab-Regular+tabdig', 'Roboto Slab', serif;">__VA_ARGS__</span>
#define DATE(preT, postT) <date class="roboto-unscale math-font-normal" datetime=STR(preT##T##postT) title=STR(preT##T##postT)>preT</date>
<ul style="padding-left: 0; list-style-type: none;">
	<li><a href="//github.com/gfx-rs/wgpu/pull/7164"              >DATE(2025-02-17, 13:24:49Z)	<strong>gfx-rs/wgpu</strong>#DIG(7164)</a>:<!--"--> Fix examples links in README<!--"--></li>
	<p></p>

	<li>
		HEADING_S(4, network-effect-vulkanalia, class="continuing", KyleMayes/vulkanalia<span style="font-weight: initial;">:</span>)
		<ul style="padding-left: 1em;">
			<li><a href="//github.com/KyleMayes/vulkanalia/pull/311"  >DATE(2025-02-17, 17:42:18Z)	#DIG(311)</a>:<!--"--> <samp>App:[:]create</samp><!--"--></li>
			<li><a href="//github.com/KyleMayes/vulkanalia/issues/312">DATE(2025-02-22, 17:15:41Z)	#DIG(312)</a>:<!--"--> <samp>queue_wait_idle()</samp> returns <samp>Err(ErrorCode(2))</samp> which it doesn't know how to format and resorts to just "2". is this <samp>SuccessCode::TIMEOUT</samp> (2)?<!--"--></li>
			<li><a href="//github.com/KyleMayes/vulkanalia/issues/313">DATE(2025-02-22, 17:43:14Z)	#DIG(313)</a>:<!--"--> All upstream documentation links are 404s<!--"--></li>
			<li><a href="//github.com/KyleMayes/vulkanalia/pull/314"  >DATE(2025-02-22, 18:01:14Z)	#DIG(314)</a>:<!--"--> Fix upstream links being 404s by linking to <samp class="path">/latest/</samp> instead of <samp class="path">/1.4-extensions/</samp><!--"--></li>
			<li><a href="//github.com/KyleMayes/vulkanalia/issues/315">DATE(2025-02-23, 20:25:56Z)	#DIG(315)</a>:<!--"--> <samp>ShaderModuleCreateInfoBuilder</samp> takes <samp>.code(&[u32])</samp> but then <samp>.code_size()</samp> in bytes, should probably just take <samp>.code(&[u32])</samp> and derive <samp>code_size</samp> automatically<!--"--></li>
			<li><a href="//github.com/KyleMayes/vulkanalia/pull/316"  >DATE(2025-02-23, 23:42:53Z)	#DIG(316)</a>:<!--"--> Use <samp>Result</samp>'s <samp>Debug</samp> and <samp>Display</samp> implementations for <samp>SuccessCode</samp> and <samp>ErrorCode</samp><!--"--></li>
			<li><a href="//github.com/KyleMayes/vulkanalia/pull/320"  >DATE(2025-02-24, 05:48:00Z)	#DIG(320)</a>:<!--"--> <samp>ShaderModuleCreateInfoBuilder::code()</samp> sets both <samp>code</samp> and <samp>code_size</samp>, remove its <samp>::code_size()</samp><!--"--></li>
			<li><a href="//github.com/KyleMayes/vulkanalia/pull/321"  >DATE(2025-02-24, 05:58:51Z)	#DIG(321)</a>:<!--"--> resetting command buffers, not pools<!--"--></li>
		</ul>
	</li>
	<p></p>

	<li><a href="//github.com/google/shaderc/issues/1484"         >DATE(2025-02-25, 11:28:24Z)	<strong>google/shaderc</strong>#DIG(1484)</a>:<!--"--> Missed optimisation: doesn't elide <samp>&lt;&lt;0</samp>/<samp>-0</samp>/<samp>>>0</samp>/<samp>|0</samp> like it does <samp>+0</samp><!--"--></li>
	<li><a href="//github.com/KhronosGroup/SPIRV-Tools/pull/6013" >DATE(2025-02-25, 15:18:23Z)	<strong>KhronosGroup/SPIRV-Tools</strong>#DIG(6013)</a>:<!--"--> optimiser: remove <samp>OpBitwiseOr</samp> <samp>OpBitwiseXor</samp> <samp>OpShiftRightLogical</samp> <samp>OpShiftRightArithmetic</samp> <samp>OpShiftLeftLogical</samp> <samp>OpIAdd</samp> <samp>OpFAdd</samp> <samp>OpISub</samp> <samp>OpFSub</samp> with 0 or vec[any](0) on rhs and <samp>OpBitwiseOr</samp> <samp>OpBitwiseXor</samp> <samp>OpIAdd</samp> <samp>OpFAdd</samp> on lhs<!--"--></li>
	<li><a href="//github.com/KhronosGroup/SPIRV-Tools/pull/6020" >DATE(2025-02-28, 21:20:44Z)	<strong>KhronosGroup/SPIRV-Tools</strong>#DIG(6020)</a>:<!--"--> Fold <samp>0 &lt;&lt;,>>,/,% n</samp> to 0. Fold <samp>a / 1</samp> to <samp>a</samp>. Fold <samp>a % 1</samp> to <samp>0</samp>. Fold <samp>f % 1.0</samp> to <samp>0.0</samp>. Fold <samp>0.0 % f</samp> to <samp>0.0</samp><!--"--></li>
	<p></p>

	<li><a href="//github.com/jakubcerveny/gilbert/pull/17"       >DATE(2025-02-26, 17:22:28Z)	<strong>jakubcerveny/gilbert</strong>#DIG(17)</a>:<!--"--> Port to Rust<!--"--></li>
	<p></p>

	<li>
		HEADING_S(4, network-effect-rust, class="continuing", rust-lang/rust<span style="font-weight: initial;">:</span>)
		<ul style="padding-left: 1em;">
			<li><a href="//github.com/rust-lang/rust/issues/137490"   >DATE(2025-02-23, 17:23:18Z)  #DIG(137490)</a>:<!-- --> What does "before-main stdio file descriptors are not guaranteed to be open on unix platforms" mean?</li>
			<li><a href="//github.com/rust-lang/rust/pull/137492"     >DATE(2025-02-23, 17:59:26Z)	#DIG(137492)</a>:<!--"--> libstd: rustdoc: correct note on fds 0/1/2 pre-main<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/pull/137493"     >DATE(2025-02-23, 18:15:27Z)	#DIG(137493)</a>:<!--"--> configure.py: don't instruct user to run nonexistent program<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/pull/137494"     >DATE(2025-02-23, 18:44:53Z)	#DIG(137494)</a>:<!--"--> libstd: init(): <samp>dup()</samp> subsequent <samp class="path">/dev/null</samp>s instead of opening them again<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/issues/137499"   >DATE(2025-02-23, 22:26:41Z)	#DIG(137499)</a>:<!--"--> nightly cargo can no longer build anything due to error: failed to create directory `<samp class="path">...\target</samp>` Caused by: Incorrect function. (os error 1)<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/pull/137547"     >DATE(2025-02-24, 16:19:37Z)	#DIG(137547)</a>:<!--"--> Revert 00bf74da6d1e8e49b18c0203e0cbd993fdac5663<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/issues/137678"   >DATE(2025-02-26, 14:57:06Z)	#DIG(137678)</a>:<!--"--> 1 million item array spends inordinate time on error path of typechecking<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/issues/137680"   >DATE(2025-02-26, 15:15:03Z)	#DIG(137680)</a>:<!--"--> Dumps unbounded amounts of data in diagnostics (27M in three lines for one error!) for large input lines (and thinks about them for minutes first)<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/issues/137681"   >DATE(2025-02-26, 15:24:24Z)	#DIG(137681)</a>:<!--"--> Deleting newlines from input makes it build 3% faster end-to-end<!--"--></li>
			<li><a href="//github.com/rust-lang/rust/issues/138953"   >DATE(2025-03-25, 23:45:43Z)	#DIG(138953)</a>:<!--"--> <samp>f32 += f32 * u32</samp> is faster in a loop than <samp>f32 += f32</samp>; can be defeated (a little bit) with <samp>#[cold]</samp> annotation?<!--"--></li>
		</ul>
	</li>
	<p></p>

	<li>
		HEADING_S(4, network-effect-sourcehut, class="continuing", ~sircmpwn/sourcehut<span style="font-weight: initial;">:</span>)
		<ul style="padding-left: 1em;">
			<li><a href="//todo.sr.ht/~sircmpwn/git.sr.ht/380">DATE(2025-03-05, 22:07:26Z)	<strong>~sircmpwn/git.sr.ht</strong>#DIG(380)</a>:<!--"--> Renaming private repository leaks old and new names into redirect table queriable at unlisted-equivalent visibility<!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<5webr4rzb2eulturkipecdkwms6m7kk2yzafse4p7jybaoknzz@tarta.nabijaczleweli.xyz>"              >DATE(2025-03-06, 05:57:04+0100)<!--"-->	[PATCH <strong>   git.sr.ht</strong>]</a> Add git.sr.ht-http-clone to intercept git http-backend explicitly and provide redirections<!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<qegljwzh2w5svuyfrajisowprqiv36ukvtomtvyas57roxuoki@tarta.nabijaczleweli.xyz>"              >DATE(2025-03-06, 05:57:08+0100)<!--"-->	[PATCH <strong>  sr.ht-docs</strong>]</a> git.sr.ht: use <samp>git.sr.ht-http-clone</samp><!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<tl3now4lhsxzx6qmordf7iemxnpbvcucypifh7lubwdax3f2ta@tarta.nabijaczleweli.xyz>"              >DATE(2025-03-06, 06:13:03+0100)<!--"-->	[PATCH <strong>   git.sr.ht</strong>]</a> shell: return "Access denied." for redirects to private repositories the pusher can't access<!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<zwwyjz6ymcroboy2cenyuahicmp3q3p7hyjykar3qqfpoz6cbb@tarta.nabijaczleweli.xyz>"              >DATE(2025-03-10, 22:29:45+0100)<!--"-->	[PATCH <strong>     core-go</strong>]</a> client: make InternalAuth.Name omitempty to allow <samp>@anoninternal</samp> auth<!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<xsxu6yn6m6khb77nkpnbmbtdskzflkcqooc4r5sqni36v3evdk@tarta.nabijaczleweli.xyz>"              >DATE(2025-03-11, 17:04:11+0100)<!--"-->	[PATCH <strong>builds.sr.ht</strong>]</a> Use <samp>git -C</samp>/<samp>hg --cwd</samp> instead of cd &&<!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<f9ba09981669ba483564d69ae5c04b3df636d1e4.1744311239.git.nabijaczleweli@nabijaczleweli.xyz>">DATE(2025-04-10, 20:54:23+0200)<!--"-->	[PATCH <strong>   git.sr.ht</strong> <!--"-->DIG(1)/DIG(4)<!--"-->]</a> commit: <samp>range(max(a, b))</samp>+<samp>if(i >= a || i >= b)break</samp> => <samp>range(min(a, b))</samp><!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<7703a13a8ee02f576549603dd5cd8d297906da94.1744311239.git.nabijaczleweli@nabijaczleweli.xyz>">DATE(2025-04-10, 20:54:25+0200)<!--"-->	[PATCH <strong>   git.sr.ht</strong> <!--"-->DIG(2)/DIG(4)<!--"-->]</a> commit: use saved variables for <samp>old_path</samp>/<samp>new_path</samp><!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<69e963880cef3e9cd3785064972a0a3271155a62.1744311239.git.nabijaczleweli@nabijaczleweli.xyz>">DATE(2025-04-10, 20:54:27+0200)<!--"-->	[PATCH <strong>   git.sr.ht</strong> <!--"-->DIG(3)/DIG(4)<!--"-->]</a> commit: detect common suffix in addition to prefix<!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<06e629292e656dc4ea766e6c2c4dde507772f60d.1744311239.git.nabijaczleweli@nabijaczleweli.xyz>">DATE(2025-04-10, 20:54:29+0200)<!--"-->	[PATCH <strong>   git.sr.ht</strong> <!--"-->DIG(4)/DIG(4)<!--"-->]</a> commit: also link renames<!--"--></li>
			<li><a href="//lists.sr.ht/~sircmpwn/sr.ht-dev/<rf5al4vxjppm6a7rxu6mumdnjol2hihgm3gzhmt3vqeoye3hti@tarta.nabijaczleweli.xyz>"              >DATE(2025-04-10, 21:28:38+0200)<!--"-->	[PATCH <strong>lists.sr.ht</strong>]</a> patchset: accumulate authors correctly instead of undecoded<!--"--></li>
		</ul>
	</li>
	<p></p>

	<li><del><a href="//github.com/Rust-SDL2/rust-sdl2/pull/1468"     >DATE(2025-03-19, 20:17:57Z)	<strong>Rust-SDL2/rust-sdl2</strong>#DIG(1468)</a>:<!--"--> Fix <samp>SDL_PIXELFORMAT_{RGBA,ARGB,BGRA,ABGR}32</samp> being backward on big-endian targets<!--"--></del></li>
	<li><a href="//github.com/pygments/pygments/pull/2881"            >DATE(2025-03-30, 13:40:29Z)	<strong>pygments/pygments</strong>#DIG(2881)</a>:<!--"--> Guesses HLSL code as "tsql" instead of HLSL/C/C++ despite "tsql" yielding errors<!--"--></li>

	<p>Bonus bugs from writing the <em>post</em>:</p>

	<li>
		HEADING_S(4, network-effect-mathjax, class="continuing", mathjax/MathJax<span style="font-weight: initial;">:</span>)
		<ul style="padding-left: 1em;">
			<li><a href="//github.com/mathjax/MathJax/issues/3342"        >DATE(2025-03-16, 00:33:04Z)	#DIG(3342)</a>:<!--"--> AsciiMath input partially explodes on matrices, TeX handles them fine<!--"--></li>
			<li><a href="//github.com/mathjax/MathJax/issues/3343"        >DATE(2025-03-16, 13:37:05Z)	#DIG(3343)</a>:<!--"--> AsciiMath turns <samp>f(\(a, b\))</samp> into <samp>f(a b)a, b)</samp>?<!--"--></li>
			<li><a href="//github.com/mathjax/MathJax/issues/3344"        >DATE(2025-03-17, 13:41:47Z)	#DIG(3344)</a>:<!--"--> Matrices render goofy in CHTML, fine with SVG<!--"--></li>
			<li><a href="//github.com/mathjax/MathJax/issues/3345"        >DATE(2025-03-17, 14:44:24Z)	#DIG(3345)</a>:<!--"--> Opening page with SVG renderer explodes the parser on <samp>\color</samp> statements; setting to CHTML and reloading works, and then setting to SVG draws mostly-correctly<!--"--></li>
			<li><a href="//github.com/mathjax/MathJax/issues/3354"        >DATE(2025-04-06, 14:24:21Z)	#DIG(3354)</a>:<!--"--> ^F search highlight broken<!--"--></li>
			<li><a href="//github.com/mathjax/MathJax/issues/3355"        >DATE(2025-04-06, 14:27:08Z)	#DIG(3355)</a>:<!--"--> ^F search "whole word" mode broken<!--"--></li>
			<li><a href="//github.com/mathjax/MathJax/issues/3357"        >DATE(2025-04-08, 21:59:53Z)	#DIG(3357)</a>:<!--"--> <samp>\overline</samp> displayed varyingly as an overline or as nothing depending on font size<!--"--></li>
		</ul>
	</li>
	<p></p>

	<li><a href="//bugs.debian.org/1101435"                           >DATE(2025-03-27, 15:46:56+01:00)	#DIG(1101435)</a>:<!--"--> /usr/bin/<strong>pyftsubset</strong>: ModuleNotFoundError: No module named 'zopfli' with <samp>--flavor=woff</samp><!--"--></li>
	<li><a href="//bugzilla.mozilla.org/show_bug.cgi?id=1957125"      >DATE(2025-03-28, 11:17 PDT     )	Bug DIG(1957125)</a>:<!--"--> Link/object bounding box no longer matches the reality of the link/object<!--"--></li>
	<li><a href="//bugzilla.mozilla.org/show_bug.cgi?id=1957298"      >DATE(2025-03-30, 09:34 PDT     )	Bug DIG(1957298)</a>:<!--"--> When updating URL with <samp>#:~:text=</samp> anchor, highlight is added for the new referenced text, but it does not replace the previous ones<!--"--></li>
	<li><a href="//bugzilla.mozilla.org/show_bug.cgi?id=1960124"      >DATE(2025-04-11, 20:20 PDT     )	Bug DIG(1960124)</a>:<!--"--> "<code>data:text/html,&lt;table>&lt;caption>a&lt;caption>b&lt;td>c&lt;/table></code>" renders only <samp>a\nc</samp> — more than 1 <code>display: table-caption;</code> element is eaten<!--"--></li>
	<li><a href="//bugzilla.mozilla.org/show_bug.cgi?id=1960952"      >DATE(2025-04-16, 09:31 PDT     )	Bug DIG(1960952)</a>:<!--"--> Ctrl+H creates history menu, then Ctrl+H again doesn't make the menu go away<!--"--></li>
	<li><a href="//bugzilla.mozilla.org/show_bug.cgi?id=1961886"      >DATE(2025-04-22, 10:49 PDT     )	Bug DIG(1961886)</a>:<!--"--> Table &lt;caption> children aren't focusable with Tab<!--"--></li>
	<li><a href="//github.com/PrismJS/prism/pull/3881"                >DATE(2025-03-30, 13:24:36Z     )	<strong>PrismJS/prism</strong>#DIG(3881)</a>:<!--"--> HLSL <samp>#define</samp> not highlighted right (... or indeed overhighlighted): uniform <samp>.token.property</samp> applies, washing out the expression<!--"--></li>
	<p></p>

	<li>
		HEADING_S(4, network-effect-blender, class="continuing", blender/blender<span style="font-weight: initial;">:</span>)
		<ul style="padding-left: 1em;">
			<li><a href="//projects.blender.org/blender/blender/issues/136787">DATE(2025-03-31, 18:33:53+02:00)	#DIG(136787)</a>:<!--"--> Default EEVEE renderer crashes-to-desktop when opening Material or Shading tabs<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/issues/136788">DATE(2025-03-31, 18:35:52+02:00)	#DIG(136788)</a>:<!--"--> 4.4.0 doesn't start at all<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/pulls/136803" >DATE(2025-04-01, 02:37:55+02:00)	#DIG(136803)</a>:<!--"--> Partially parse AMD driver versions to determine if they require legacy work-arounds instead of spot-checking<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/pulls/136839" >DATE(2025-04-01, 16:53:54+02:00)	#DIG(136839)</a>:<!--"--> Fix Makefile error erroring without the error message on Win32<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/issues/137112">DATE(2025-04-07, 21:10:58+02:00)	#DIG(137112)</a>:<!--"--> The input map at the bottom of the screen doesn't indicate you can write Δxyz directly in Move operation (nothing else does, either, but this is where you'd expect it)<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/issues/137113">DATE(2025-04-07, 21:15:40+02:00)	#DIG(137113)</a>:<!--"--> Impossible to tab backward when writing Δxyz directly in Move operation<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/pulls/137124" >DATE(2025-04-08, 05:24:43+02:00)	#DIG(137124)</a>:<!--"--> Fix #137113: bind universal Shift+Tab to go back instead of Ctrl+Tab<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/issues/137357">DATE(2025-04-11, 17:52:21+02:00)	#DIG(137357)</a>:<!--"--> Minimum accepted OpenColorIO, wayland-protocols, libvulkan-dev, OpenImageIO versions too low?<!--"--></li>
			<li><a href="//projects.blender.org/blender/blender/issues/137749">DATE(2025-04-18, 23:58:29+02:00)	#DIG(137749)</a>:<!--"--> Keyframes appear and disappear in the Timeline window depending on where the scrub head is, no filters applied <!--"--></li>
		</ul>
	</li>
	<p></p>

	<li><a href="//github.com/servo/servo/issues/36480"               >DATE(2025-04-12, 02:53:08Z)	<strong>servo/servo</strong>#DIG(36480)</a>:<!--"--> Links stop being links when... big page? or maybe some javascript?<!--"--></li>
	<li><a href="//github.com/servo/servo/issues/36481"               >DATE(2025-04-12, 03:04:39Z)	<strong>servo/servo</strong>#DIG(36481)</a>:<!--"--> Crashes for a bit then segfaults on MathJax-heavy page (also cites min-/max-content size. idk)<!--"--></li>
	<li><a href="//github.com/servo/servo/issues/36650"               >DATE(2025-04-22, 12:26:17Z)	<strong>servo/servo</strong>#DIG(36650)</a>:<!--"--> Segfaults on exit based on page contents(?) on Win32<!--"--></li>
	<li><a href="//github.com/matplotlib/matplotlib/issues/29912"     >DATE(2025-04-13, 17:32:23Z)	<strong>matplotlib/matplotlib</strong>#DIG(29912)</a>:<!--"--> [Bug]: findfont: Font family '&lt;font>' not found. despite being present and queriable via fontconfig; never makes it to <samp class="path">~/.cache/matplotlib/fontlist-v330.json</samp> either<!--"--></li>
	<li><a href="//github.com/matplotlib/matplotlib/issues/29913"     >DATE(2025-04-13, 18:00:50Z)	<strong>matplotlib/matplotlib</strong>#DIG(29913)</a>:<!--"--> [Bug]: <code>plt.ticklabel_format(style='plain')</code> puts the y axis label out of frame<!--"--></li>
	<li><a href="//bugs.kde.org/show_bug.cgi?id=503481"               >DATE(2025-04-28, 15:20Z   )	Bug DIG(503481)</a>:<!--"--> Moving cursor around in-image WYSIWYG copies text colour under cursor to everything else<!--"--></li>
	<li><a href="//bugs.kde.org/show_bug.cgi?id=503482"               >DATE(2025-04-28, 15:26Z   )	Bug DIG(503482)</a>:<!--"--> Subscript doesn't render correctly<!--"--></li>
</ul>
